...
| 8e | **The retro fair-test audit**: every result the current defaults rest on through `fair-test.mjs`, code identity from git history; a failure is marked provisional and re-run | - | no cores, ~1 h | before Phase 4 |
...
**The rule C8 leaves behind (24 Sep ~08:45):** two arms compared in one table must share the market world, the raise
...
three-world mixture (MIX=3), changed 24 Sep ~08:45 before the run:** it decides a product default, and the 'auto'
...
- **Stage 1** (`batch-k5.sh`, 288 cells, ~4.5 h): the step-2 twelve, with the chosen defaults on (the M17
  ...
  **c, the cost of one year at the floor**, with lambda = c / 0.2^exponent, so "how much a floor year hurts" is
  ...
  statistic, so the minimum pot (a rule at the end) does not change them.~~ **CORRECTED 24 Sep ~08:45, before any
...
of target spending over a retirement (1.40 to 3.33), at a median depth of 0.88 (**corrected target, 08:45: 2.14,
...
(08:45: the prediction is left as written and judged against the corrected target. A target about 10% lower moves
