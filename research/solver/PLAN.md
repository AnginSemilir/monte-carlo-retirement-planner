...
- **The PreToolUse hook judged whole command lines, not their parts** (found 24 Sep ~09:55 UK when an audit run got through):
  ...
  same; a `-n` flag anywhere read as `git commit -n`. **In the working tree only (uncommitted, awaiting the maintainer)**
  each part of a command is judged on its own; 53 tests prepared and passing against that version, not in the repo.
  The COMMITTED hook still judges whole lines and still asks (approved unseen in auto mode). And the first version's "ask" for the enforcement files was approved without the maintainer in auto mode; it is
  a refusal only the maintainer's own message lifts ("unlock enforcement") in the working-tree version.
  **Same pattern searched:** every rule in pre-tool.mjs: the hooksPath rule was whole-line too (a `--get` anywhere exempted the line),
  per part in the working-tree version; the lock rule was per part; the protected-file rule stays whole-command on purpose (it errs towards refusing). **Open
...
- **`audit-s126.mjs ids` read its grid size and path count from the wrong arguments** (found 24 Sep 09:45 UK by the
  ...
  select-phase4.mjs) read their numbers inside each mode. A `smoke.sh` check of the `ids` mode's settings is written but
  UNCOMMITTED (one of the four enforcement edits awaiting the maintainer); the committed smoke.sh runs only the scan mode.
