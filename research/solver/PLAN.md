...
|---|---|---|---|
| 24 Sep 12:24 | **The F1 test read against its prediction: NOT FALSIFIED** - PROVISIONAL (it ran before result files carried their code: commit 6dd1181, established by hand until the retro audit 8e). Item 1 held on 11 of 13 in-class cases (S130 +5.3, just outside +/-5; bridge 6 -46.0, outside the edge's +/-15); item 2 held (the nearest, bridge 6, -0.5 +/- 0.26); item 3 missed on 6 of 9 (the thin households keep the pension below its tier); item 4 held; item 5's premise wrong (F1 acts on the long bridges: S360 +3.9 +/- 0.64). The class column re-read at the floor need (O8 fixed: share 0.95 and bridge 6 are in the class) | O3 and O8 resolved; O2, O4, O5 and O9 stay open, gated by the pitfall sweep (8d); F2 is not built (the falsifier did not fire); **whether F1 becomes the default is the maintainer's decision** (asked 24 Sep ~12:17 UK) | results: results-f1-verdict.txt, results-f1.txt; fair-test: pass, by hand (fair-gate cannot read a text log): both arms in one process on the same 1,000 paths, paired; the prediction's 33-row table written before the run; the code by hand (6dd1181); prediction: predictions/f1-test.md |
| 24 Sep 12:24 | **experiment.mjs refuses an unknown setting** (the plan-auditor's tenth review, 12:16 UK: PLANTIER was not the only setting that fell back silently - FAILSHORT, SHAREDEAD, BEQSHAPE, RESIL and every on/off flag did too, while the result file recorded the value as typed): `settings.mjs` checks every word, flag and number before a run and stops on a bad one; settings.test.mjs refuses each planted value and accepts all 28 the scripts pass | the PLANTIER bug fixed now, not held; the code hash moves from f15075f2eef3 to a26eabb0e3e9 (a guard that only refuses: a valid run is unchanged) - K5's later stages and M14b declare it in their fair-test row 28; the smoke run passed on it (12:23 UK) | results: none (a build); fair-test: n/a (a build: each planted value refused); prediction: none (a build) |
...
| O2 | F1 leaves long bridges misread: bridge 6 (table 53.3 against 99.3 simulated), S360 (4.3 against 44.1), S366 (3.0 against 99.2) | 24 Sep, F1 test | Claude | the pitfall sweep (8d), before Phase 4 (left open by the F1 write-up, 12:24 UK: results-f1-verdict.txt) | open |
...
- **`runFixedPath` called `world(...)` with an undefined `act`** (the M15 edit replaced both `world` calls in
  ...
  smoke.sh before a batch that uses it runs on edited code: the stamp re-runs the smoke test, not every mode.
...
- **An unknown tier name silently becomes the top tier** (found 24 Sep 12:01 UK by the plan-auditor, in stage 3b's
  ...
  unknown name already failed loudly. settings.test.mjs refuses each planted value and accepts all 28 the scripts pass.
...
| 8d | **The pitfall sweep** (C1-C5, C7 and C8 above): the S126 fix, then the same pattern hunted, tested and fixed (an edit to audit-s126.mjs now re-runs the launcher's smoke run). **Gates Phase 4** | S126 fix | ~4-6 h, in run gaps | Thu evening / Fri morning |
...
| 9 | **Phase 4**, with its bundled extras (below); M23 decided (A, built): arm A runs with `raiseCap: 1.1`. **Waits for the pitfall sweep (8d) and the retro fair-test audit (8e)** | 8, M23 | ~6 h | Fri ~01:00 |
...
**F1 TEST - THE RESULT (read 24 Sep 12:24 UK by `read-f1.mjs` over `results-f1.txt`; `results-f1-verdict.txt`): NOT
FALSIFIED, PROVISIONAL** (the run predates the code stamps; its code, 6dd1181, is established by hand until 8e). The class
...
2. **No survival cost: held.** The nearest is bridge 6, -0.5 +/- 0.26 (1.9 se).
3. **Behaviour: missed on 6 of 9.** Years below tier fall on S126 (40.0 -> 10.7), S120 and wealth x2, but not on the thin
   households: S124 39.1 -> 39.1, S128 28.1 -> 29.7, S130 35.4 -> 36.9, bridge 4 42.0 -> 42.0, wealth x0.5 36.8 -> 39.4 (O5).
...
paired se), so F2 is not built. What F1 leaves - long bridges misread, the thin households' tier, a slightly optimistic
read - is open in the register and gated by the pitfall sweep (8d). **Decision for the maintainer:** make F1 the default.
Recommended, since it closes the dominant misread at no measured survival cost and the table is never a reported number;
if yes, it lands after K5's stages 1 and 2 (or once F1 is shown not to act on K5's twelve), so their files stay comparable.
...
**Gate:** Phase 4 does not start until C1-C5, C7 and C8 are each cleared or fixed, and re-tested with the fix (C7 added 24 Sep 09:48 UK after the plan-auditor's review; C6 alone is deferred, to the couples work).
