...
- **The PreToolUse hook judged whole command lines, not their parts** (found 24 Sep ~09:55 UK when an audit run got through):
  ...
  origin, so it can never unlock; since a00f4b1, 26 Sep, the same message absorbed into the turn is recorded with its typist's origin and does). Each planted form: allowed by the committed hook, refused by the new one
