...
| the purpose test (M18) | at 160 positions on 8 households (half on the cliff), the solver's pick is the best-surviving of its top six distinct moves within noise at 159; the table's order tracks simulation; a small lean toward caution in comfortable positions | `results-bestof.txt` |
...
| ~03:30 - ~04:05 | **Probe M15** (`batch-m15.sh`, 12 cells: the six households with the option on against s2-fnewex, and the three GIA-heavy ones at 40% gain off and on); prediction as written in the M15 row, unchanged | M18-floor reduced |
