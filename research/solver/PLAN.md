...
|---|---|---|---|
| 25 Sep 21:54 | (maintainer) **The regimen's enforcement applied** ("unlock enforcement", 21:54 UK, sent between turns after four typed mid-turn, 20:54 to 21:48 UK, reached the hook as queued messages) | check-prediction.mjs holds new predictions to the regimen's fields (the nine earlier exempt by name); check-plan.mjs lints claims, grades new ledger rows and no-effect evidence, and allows "noted, below materiality"; plan-defaults.test.mjs pins the margins; the launcher re-runs each derive line; fair-gate.mjs reads audit-s126.mjs's stamped logs (requireFairLogs), its test accepts it, reduce-o22.mjs is legacy, and code-id.mjs stamps imported modules - CI's red since 06:00 UK ends (the bugs list); the pre-commit hook runs CI's four tests (the maintainer's decision 4, taken by default); the checklist carries the regimen (PLAN.md's copy with it). Refused by the session's permission classifier as changes to the assistant's own configuration, and left for the maintainer: the hook's mid-turn unlock and its tests, the reviewer's tiers (.claude/agents/plan-auditor.md) and the CLAUDE.md line; the drafts are in the session's scratchpad. Not built: the seed registry (a Seeds field, decision 3) | decision: maintainer 25 Sep 21:54 UK |
...
| 8f | **The combined no-harm run (new, the maintainer 25 Sep 07:29 UK):** every new default together - 'auto' at 85%, the exact final year (DECIDED 25 Sep 09:36 UK; since solvePlan now runs it by default, the previous-baseline arm passes `finalIntegral: false` explicitly), the bridge fix 7e chooses, the dislike-of-cuts setting (O15) and five worlds only if O21 has adopted them by then - against the previous baseline, paired, on a broad panel of ordinary library households chosen by a rule that never looks at the solver, with survival, estate and the cut term reported per household; its decision rule is the regimen's (RULES.md section 8; the maintainer 25 Sep 22:01 UK): survival primary, Holm across the households, the margins (0.25 points where the previous baseline survives 95% or more, 0.5 below), a random-effects pooled mean with its floor of -0.1 points, spending, cuts and estate descriptive, the risk-based arm and year-on-year falls reported beside it (8i), set in its prediction, registered first (a plain "no household beyond two se" would fire by chance about half the time at 30 households, the thirty-ninth review); then R8 re-derived. If it shows problems, couples (O11) and the end-of-plan reader (the outside reviewer's joint bridge-and-terminal template, outside-review-reply-2.md) come back before Phase 4. Why: each default was tested alone and mostly on households chosen because they misbehaved. **Decided by the maintainer, 07:50 UK ("I approve your new recommendations"):** the pitfall sweep (8d) runs first, since it can change code this run should cover, and O21 first, so an adopted five-world setting is in it | 7e, 7k, O15, O21, 8d | sized when the panel is fixed | after 8d |
...
| 8g | **Perturbed engines and the first error-budget entry** (the review's Finding 5 and section 14): the survival change under 2c.1's existing perturbed engines on the twelve step-2 households (about an hour on four cores), then the two-factor, fat-tailed-with-inflation-shocks and bootstrap engines built for Phase 4's condition 4 | 7e | ~1 h, then the builds (not sized) | before Phase 4 |
...
3. **No household badly worse:** none shows harm at a 1-point survival margin by the exact rule (Holm across the panel,
   25 Sep 22:01 UK), and none loses more than 5% of spending delivered.
