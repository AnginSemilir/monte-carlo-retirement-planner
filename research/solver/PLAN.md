...
- **`runFixedPath` called `world(...)` with an undefined `act`** (the M15 edit replaced both `world` calls in
  ...
  audit-s126.mjs's new f1v2 mode, which the batch 7c runs - so 7c does not launch until it has one. **Proposed to the
  maintainer (not yet approved):** add settings.mjs and code-id.mjs to the stamp; read the version in fair-gate's row 24;
  add an f1v2 line to smoke.sh.
...
| 7c | **The F1 v2 test** (approved 12:42 UK): off against v2, paired, on the step-6 defaults **in the mixture** - the twelve S126 variants, S120-S130, S360, S366 and S370; its prediction (`predictions/f1v2-test.md`) written and pushed before it launches. Settles O2, O4 and O9 or leaves them open; whether F1 becomes the default waits for it and for 8d | 7 stage 1; F1 v2 built; an f1v2 line in smoke.sh (the maintainer's unlock) | ~1 h (the F1 test took about 5 min a case in the fold, x2.06 for the mixture - the stage-3b estimate's ratio - so ~10 min a case, five cases to each of four processes), re-estimated from its first cases | after K5 stage 1 and M14b, or before M14b if the smoke line is in first |
| 7d | **Couples in a bridge (O11)**, then **the thin households' lower tier (O5)**: is either a second misread? Each measured paired, with its prediction first; F2's design written alongside, no cores (`f2-design.md`, 13:16 UK: leaning to a coverage coordinate in bridge years, with a node at coverage 1), and chosen between after 8d | 7c | not estimated yet | after 7c |
...
- **Money arriving later in the bridge is not counted.** F1's coverage test adds known costs but not known inflows, while
  ...
  - **The bridge-6 variant** also receives one inside its bridge, but the test counts it just covered without it
    ...
    simulated (the no-growth cap reproduces it at 54.5; with growth alone it would read 75.6, so the rest is the
    uncounted inheritance).
