...
| O13 | K5 stage 1: the solver cuts at most 37% of the guardrails' total even at c = 0.0001, and raises about three times as much, for survival at or above theirs on 11 or 12 of 12 (results-k5-stage1.txt). Why is NOT CHECKED: the likely reading is that a cut earns nothing in the solver's objective unless it raises survival | 24 Sep 14:38 UK, K5 stage 1 | Claude | the maintainer's decision on Phase 4's fairness condition (option B would check it), before Phase 4 | open |
...
(ACCEPTED before the run); the cells' code is established by hand (one snapshot of 31a1b52, 7824be314568).
...
Raises run 3.17 to 3.33 years against the guardrails' 1.07 at every point, so the solver both cuts far less and raises far
more, for survival at least as good. **Why is NOT CHECKED** (O13): the likeliest reading is that the solver has no
reason to cut more - a cut pays in its objective only by raising survival, and these households already survive at or
above the guardrails' rate - while the guardrails cut by rule, whether or not it helps.

**Decision for the maintainer (put 14:38 UK):** stages 2, 3 and 3b, K6 and M15 v2 all need K5's point and are
held. Phase 4's fairness condition was "the solver cuts about as much as the guardrails, so its survival is not bought with
spending"; the data turn that worry round - the solver spends MORE (fewer cuts, more raises) and survives at least as well.
(A) **Recommended:** replace the cut-matching condition with the backstop already in Phase 4, total spending delivered: the
solver must deliver at least the guardrails' spending, household by household, and survival is compared at that; the
dislike-of-cuts default is then set on its own terms (K6's spread around a stated reference, not a guardrail match).
(B) Before deciding, a small diagnostic on two of the twelve at c = 0.0001 (minutes, no batch): does a cut ever raise
the solver's survival there? It would confirm or refute the reading above. (C) Extend the grid to lower c: not
recommended - it would take c near 1e-8, a dislike of cuts of effectively zero.
...
unless K5 stage 2 brings the solver's raises down (R4).** The earlier text follows.
