...
| O16 | A rarely used option reshapes the everyday tier: allowed one tier above, S172 holds the plan's tier in 73.4% of years against 0.1% without (97.0% two below); S414 likewise; the thin households move the other way (S070 at the plan's tier 12.1% -> 1.2%, the bets kept for when behind) - results-m14b-why.txt. Why is NOT CHECKED (the likely reading: a later rescue makes a bad outcome look less bad in the table, whose bad positions read optimistic, M16). M14c adds S162: its 6 bets simulate better than staying (+2.33 +/- 0.47, results-m14c.txt), yet M14b lost it 7 paths, so the option's cost there lies on paths that never bet - the option changing everyday decisions, this row's mechanism | 24 Sep 16:58 UK, M14b path by path | Claude | before any risk-above default reaches users; the Medium-tier diagnostic (R7) | open |
...
| O20 | **S194's bets are wrong when made, and mostly far from the end.** At its 40 first-bet positions the bet simulates worse than staying (-0.51 +/- 0.11, z -4.6) while the table ranks the bet first at all 40 (results-m14c.txt); exploratory, by years left: -1.75 +/- 0.48 at 6-10 years, -0.76 +/- 0.30 at 11-20, -0.08 +/- 0.08 in the last 5 (results-m14c-horizon.txt). The final-year staircase (O19) acts directly on the last years, so it can explain these only through the table it passes back, NOT CHECKED; the alternatives are the earlier years' averaging and the grid read (the drafted quadrature reference, Part A) | 24 Sep 22:10 UK, M14c | Claude | O19 (does S194's tier-above cost go with the exact final year?), then the quadrature reference's Part A at S194's positions | open |
