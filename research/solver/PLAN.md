...
plan-auditor's review. **Since 25 Sep 20:47 UK** (the maintainer adopted the outside review's regimen: RULES.md section 8)
a new test is read by exact tests against registered margins, with three outcomes (no material harm, harm,
inconclusive), and its prediction carries a decision rule, power, credence and a pre-mortem; results read before then
keep the reading they had, and 8e re-reads exactly the ones a default rests on.
...
{"raiseCap": 1.1, "minPotYears": 1, "estateWeightMin": 0.01, "thinSurvival": 0.85, "thinPaths": 1000, "thinSeed": 7101,
 "raiseSurvival": true, "failureShortfall": true, "finalIntegral": true, "riskAboveDefault": "auto", "giaTiers": "refused", "bridgeRead": false,
 "margins": {"high": 0.25, "low": 0.5, "pooled": 0.1, "highAt": 95}}
-->
...
|---|---|---|---|
| 26 Sep 07:10 | **8j built: the scorecard** (the maintainer's "Go ahead and build it") | research/solver/scorecard.mjs reads each registered prediction's "## Credence" (7e's parses to items 1-9 at 0.70, 0.80, 0.60, 0.65, 0.75, 0.80, 0.85, 0.75, 0.95 and the whole at 0.60) and the reducer's saved output; a test with no results file is PENDING, a reducer that stopped is REFUSED, a mismatch between credence items and outcome items stops the scorecard; Brier per test and cumulative, a reliability table in four bins. 15 planted checks run before any real file (a Brier worked by hand, a perfect forecaster 0, always 50% 0.25, the PENDING, REFUSED and mismatch faults, a credence above 1); 7e is PENDING until results-7e.txt is saved | results: results-scorecard.txt (7e PENDING, no score yet); fair-test: n/a (a tool, no arms); prediction: none (a build); grade C (a build's own planted checks) |
| 26 Sep 06:40 | (maintainer) **7e's pooled floor stays fixed effect, on the corrected trade-off** (answering at 06:40 UK: "Keep fixed effect") | sim-pooled-floor.mjs now runs Holm across all 24 wave-1 cases (the eight outside the pool at the prediction's expected change) and losses over three to six cases: a small spread loss is caught 8 to 10 points less often with fixed effect (three cases each 0.3 lower: 57% against 67%; six each 0.2 lower: 76% against 84%), a gain on one case falsely fires random effects in 7 to 13.5% of runs against about 2%; the prediction, RULES.md and 8f carry it | decision: maintainer 26 Sep 06:40 UK; results: results-pooled-floor.txt (4,000 runs a scenario); fair-test: n/a (a simulation of the decision rule, no arms run); prediction: none (a derivation for a registered prediction's rule); grade C (a simulation of the rule over 7c's records) |
| 26 Sep 01:49 | (maintainer) **7e's pooled floor stays fixed effect, on the rebuilt simulation** (answering the trade-off, 01:49 UK: "Keep fixed effect") | the prediction, 8f, RULES.md and the code comments carry the rebuilt figures: with no change the whole falsifier fires in about 3% of runs either way; one pool case's 3-point gain in 7 to 13% with random effects and about 2% with fixed effect; a loss on two cases fires the fixed-effect floor less often, but the whole falsifier differs by at most about 4 points. **Corrected 26 Sep 06:40 UK (the fiftieth review, BLOCKING 1): the bound was wrong - it came from two-case scenarios and Holm across 16; see the 06:40 row** | decision: maintainer 26 Sep 01:49 UK; results: results-pooled-floor.txt (sim-pooled-floor.mjs, mulberry32, 4,000 runs a scenario); fair-test: n/a (a simulation of the decision rule, no arms run); prediction: none (a derivation for a registered prediction's rule); grade C (a simulation of the rule over 7c's records) |
| 25 Sep 22:59 | (maintainer) **The enforcement changes of 22:45 UK applied** ("unlock enforcement", delivered 22:59 UK; answering "Yes" and "Try them once more" at 22:45 UK) | .githooks/pre-commit keeps a staged prediction's own name (the bugs list) and builds the engine before CI's tests (4c42eda; shown on a planted incomplete prediction and a planted build failure); 7e's prediction committed there with the 22:45 UK choices; .claude/hooks/pre-tool.mjs honours a mid-turn "unlock enforcement" recorded with its typist's origin, 5 checks in hooks.test.mjs, 120 passed (a00f4b1); fair-gate.mjs's and the hook's comments date CI's red from 24 Sep 14:00 UK. Refused again by the session's permission classifier and left for the maintainer: the reviewer's tiers (.claude/agents/plan-auditor.md) and the CLAUDE.md line | decision: maintainer 25 Sep 22:45 UK, unlock 22:59 UK; results: none (enforcement code; hooks.test and the planted refusals in the commit messages); fair-test: n/a (no arms compared); prediction: none (a build, not a test); grade C (a build's own checks) |
| 25 Sep 22:45 | (maintainer) **7e: look 2 at 8,000 paths on bridge 4, wealth x0.5, S130 and S128; the pooled floor by fixed effect** (answers to Claude's questions, 22:45 UK, after the forty-eighth review's BLOCKING 1 - the path count had not been put to the maintainer - and MINOR 2); also, by the same answers: the engine built in the pre-commit hook, and the refused self-configuration changes tried once more, both applied under the unlock delivered 22:59 UK (the next row) | reduce-7e.mjs reads look 2 at each case's own count (LONG) and pools by fixed effect with random effects reported beside it (stats.mjs pooledFE, 5 new tests, 28 passed); 50 planted checks, 31 mutations caught, among them the floor back to random effects and 8,000 back to 3,000; batch-7e.sh runs look 2 in two groups; derive-7e.mjs gives each case's exact lower end at its look-2 count: bridge 4, wealth x0.5 and S130 are expected to be decided at 8,000, S128 to sit at the edge (-0.512 against 0.5); the prediction's rule, power, falsifier and derive hash follow. The timing behind the choice: 92 s per 1,000 paths beside a solve of about 180 s | decision: maintainer 25 Sep 22:45 UK; results: results-look2time.txt (a timing, measured through the launcher), results-pooled-floor.txt and results-derive-7e.txt (scripts over 7c's records); fair-test: n/a (no outcome compared); prediction: none (a measurement and two derivations); grade C (descriptive; no test) |
| 25 Sep 21:54 | (maintainer) **The regimen's enforcement applied** ("unlock enforcement", 21:54 UK, sent between turns after four typed mid-turn, 20:54 to 21:51 UK, reached the hook as queued messages) | check-prediction.mjs holds new predictions to the regimen's fields (the nine earlier exempt by name); check-plan.mjs lints claims, grades new ledger rows and no-effect evidence, and allows "noted, below materiality"; plan-defaults.test.mjs pins the margins; the launcher re-runs each derive line; fair-gate.mjs reads audit-s126.mjs's stamped logs (requireFairLogs), its test accepts it, reduce-o22.mjs is legacy, and code-id.mjs stamps imported modules - CI's red ends (the bugs list: red since 24 Sep 14:00 UK, not 06:00 UK as first written; the third cause fixed in fb9ca50); the pre-commit hook runs CI's four tests (the maintainer's decision 4, taken by default); the checklist carries the regimen (PLAN.md's copy with it). Refused by the session's permission classifier as changes to the assistant's own configuration, and left for the maintainer: the hook's mid-turn unlock and its tests, the reviewer's tiers (.claude/agents/plan-auditor.md) and the CLAUDE.md line; the drafts are in the session's scratchpad. Not built: the seed registry (a Seeds field, decision 3) | decision: maintainer 25 Sep 21:54 UK |
| 25 Sep 21:14 | **Check 6: the bridge reader adds about 3 to 6% to a solve at 16 points** (a measurement, not a test: audit-s126.mjs's readertime mode through the launcher, launched 20:37 UK, a quiet box, load 1.0 to 1.6; two alternated runs each way). Medians off against the reader: S126 178.4 s against 183.4 s (ratio 1.028), bridge 6 196.5 s against 202.7 s (1.032), S366 205.6 s against 218.7 s (1.064); 0 unsupported nodes on every reader solve. Run on the code before the reader's growth-convention fix (the bugs list, 25 Sep); a re-timing on the fixed code NOT CHECKED. Not measured at the product's 30 points | predictions/bridge-reader.md's run-time derivation gets its figure; 7e's time bar is judged at 30 points inside 7e (time30.txt), so this measurement sets no bar | results: results-readertime.txt; fair-test: n/a (a timing, no outcome compared; both settings alternated on one quiet box, the load printed with each solve); prediction: none (a measurement, declared in runs.log 20:37 UK); grade C (a descriptive measurement, not a test) |
| 25 Sep 21:05 | **7e's decision rule built and shown able to fail** (a build and its planted checks, not a test result): reduce-7e.mjs reads 7e by the outside review's section 19 rule - exact one-sided McNemar per case, Holm across the 24, the margin by off's survival, two looks (1,000 paths, then 3,000 for the cases the first leaves open), the 30-point cases and the controls as their own families, a pooled bridge-class mean with its floor, the time bar, and the misread falsifier kept (the forty-sixth review's BLOCKING 1). 38 planted outcomes read as they should, and mutate-reduce-7e.sh breaks 26 parts of the rule, the gate and the completeness check, every one caught. derive-7e.mjs gives the power from 7c's records: look 1 is expected to leave 11 of the 21 recorded cases open; the least loss look 2 can call harm is 0.25 to 0.57 points on the covered cases, 0.87 on S130, 1.20 on bridge 4+cost and 1.23 on S128. **Corrected 25 Sep 22:01 UK (the forty-seventh review, two BLOCKING):** the look-2 half-width used the 1,000-path discordance at 3,000 paths; scaled, look 2 is expected to leave bridge 4, wealth x0.5, S130 and S128 open as well as the four that clear the margin only by a gain (8 in all, not 3); and the pooled floor over the mode's class flag read FALSIFIED on a run with no path lost (share 0.95's gain widening the random-effects interval), so it now pools the 16 cases expected unchanged, whose lower end under no change is -0.066 and under a 0.2-point loss on each -0.272 (results-derive-7e-2201.txt, the derivation's output as it stood then, kept verbatim from e8090fc; re-derived 22:55 UK under the maintainer's choices of 22:45 UK in results-derive-7e.txt); 46 planted checks, 27 mutations caught (results-reduce-7e-mutations.txt) | batch-7e.sh runs look 2 beside wave 2; predictions/bridge-reader.md carries the rule and the regimen's fields (decision fed, provenance, the derivation script and its hash, point and interval, credence, power, budget line, pre-mortem) and the forty-sixth review's fixes (the falsifier's class read as 7c's; completeness; the probability blend; S168 added and the low-risk case's omission declared; the O5 reading; the written times; the path count in the gate); 7e is registered once a review passes (the forty-seventh failed, 21:56 UK; fixed in the rows above) | results: results-reduce-7e-mutations.txt, results-derive-7e.txt, results-derive-7e-2201.txt; fair-test: n/a (a reducer's planted checks, no arms compared); prediction: none (a build, not a test); grade C (a build's own checks, not a test) |
| 25 Sep 20:47 | (maintainer) **The outside review's regimen adopted** ("take on board the project management recommendations - it may me changing the rules, checklist, reviewer etc.", 20:47 UK, on "Solver research: review, new findings and the updated regimen", shared 20:42 UK) | RULES.md section 8 written: exact tests against margins (0.25 points where the comparison arm survives 95% or more, 0.5 below, 0.1 for a pooled mean), Holm, three outcomes, two looks, power before a run, replication before a default, a seed registry, the prediction's new fields, evidence grades A to D, review depth by stakes, the two-source rule, a materiality gate, decision records, stop rules, the value-of-information screen and the error budget; stats.mjs with its tests; the margins in the decided-defaults block and in stats.mjs, which reduce-7e.mjs reads. The test pinning the two (plan-defaults.test.mjs), the checklist, check-prediction.mjs, check-plan.mjs, the launcher and the reviewer's instructions are locked: their changes are drafted and tested on a scratch copy and wait for the maintainer's unlock (typed three times mid-turn, 20:54, 21:12 and 21:17 UK, each reaching the hook as a queued message, which by design cannot start one). The sweep of superseded text the maintainer asked for (21:12 UK) follows with them | decision: maintainer 25 Sep 20:47 UK |
| 25 Sep 20:31 | (maintainer) **The dislike-of-cuts research reference: c = 0.001, lambda 0.025 at exponent 2** ("Approve lambda 0.025 as the research reference", 20:31 UK, on drafts/o15-reference.md: the thin households' survival knee on K5 stage 1's grid, and the lowest c whose unlucky tenth spends at or above the guardrails' on all twelve, results-o15-grid.txt) | a research reference, NOT a product default, as the draft recommended: solvePlan still requires the setting, and the code and the decided-defaults block are unchanged; it centres K6's sweep (a tenth to ten times), arm S in Phase 4 and M15 v2's probe; K6 re-checks it in the product configuration (the mixture, the exact final year, 'auto'), on the twelve at their own tier and held at Medium (stage 3b's question), on 3,000 held-out paths of seed 7011 (it was chosen on seed 7002's; seed 7012 since 25 Sep 22:01 UK, the forty-seventh review: 7011 is registered to M14b and 7e, RULES.md section 8), before any product default is decided; 7e keeps S126's held lambda (0.0223606797749979) so it stays comparable with 7c; O15 resolved | decision: maintainer 25 Sep 20:31 UK |
| 25 Sep 11:14 | (maintainer) **7e's smoke line added to smoke.sh** ("Unlock enforcement", 11:14 UK, after being told 7e's batch needs one line in the locked smoke check; the unlock used for that line only) | audit-s126.mjs gains 7e's batch mode, bridge7e (off, v1, v2 and the reader, each optionally @points, paired against the first arm; the tier above and the exact final year set explicitly in every arm; a ran line per arm); smoke.sh gains one step running it tiny (S126, the reader arm, 4 points, 20 paths) and checking its ran line for the mixture, the tier above, the exact final year and bridgeRead reader. Its three greps, copied verbatim, pass on the right output and fail on five planted faults (reader off, final year averaged, single table, no tier above, no case line); the smoke run on this code: SMOKE PASSED (25 Sep 11:25 UK, 13 steps, the new one "S126 audit, bridge7e mode, the bridge reader in the mixture" ok) | decision: maintainer 25 Sep 11:14 UK |
| 25 Sep 11:13 | **The bridge reader built into the solver (`bridgeRead: 'reader'`), and its own checks 2 to 4 passed** (a build and its unit checks, not a test result; drafts/reader-design.md, the code committed as 2a0557a): the trap reads 0.40, not the unit template's 0.85; node reproduction over 7776 nodes of S126's 6 reader tables (two bridge years, three worlds), largest miss 5.6e-17; bit for bit from the last bridge year on (S126) and everywhere on S000 (no bridge); each check shown failing on a planted fault. (Corrected 25 Sep 21:25 UK.) On S126 every nonzero residual (4698 of 7776 nodes) sits at the clamp, 1.0e-6, where p x c is 0, so dropping it reads 0 and the clamp restores the same value: that fault cannot show on S126 at any tolerance - the first run's "nothing to catch" was right in effect though judged at 1e-3, the forty-sixth review's "caught at 1e-9" missed the clamp, and the planted check re-run at 21:17 UK failed on it (results-reader-callers.txt). The dropped residual is now planted on S366, where the residual carries value, with node reproduction there too; p taken as 1 is planted on both (reader-solve 16 passed, results-reader-solve-fixed.txt); the synthetic row, computed by hand, checks buildReaderTable's split, not the read. Callers re-tested on this code: reader.test.mjs 30 passed, solver-f1.test.mjs 19 passed, f1-default.test.mjs 5 passed, solver-step2.test.mjs 7 passed (at 11:13 UK, in the transcript only; re-run 25 Sep from 21:17 UK on the code with the growth-convention fix, with the reader's own checks, read-f1v2 and solver-plan: results-reader-callers.txt) | 7e's reader dependency: built, checks 2 to 4 passed; check 6 (its run time, audit-s126.mjs readertime at 16 points) runs through the launcher next; the probes (check 5) are not built: 7e reports only each reader solve's count of unsupported nodes, which bridge7e prints; its smoke line added with 7e's batch mode under the maintainer's unlock (the row above); drafts/reader-design.md records what the build chose (the reference mix: ISA, taxable account and cash by opening balance at the plan's tiers and each world's rates) | results: results-reader-checks.txt, results-reader-callers.txt, results-reader-solve-fixed.txt; fair-test: n/a (a build's unit checks, no arms compared); prediction: none (a build, not a test); grade C (a build's own checks, not a test) |
| 25 Sep 09:36 | (maintainer) **The exact final year is the product default** ("Approve and unlock enforcement", 09:36 UK, on O19's no harm on six households, 7j's S360 - survival unchanged, end wealth higher, not registered - and 7k's 3-9% more solve time at 16 points) | solve.js PRODUCT_BASELINE finalIntegral true; the decided-defaults block; plan-defaults.test.mjs pins it (the locked test changed under the maintainer's unlock; shown failing on a planted mismatch); audit-s126.mjs's trace and time modes name the setting in every arm, and its f1v2 and quad modes now run the default; solver-plan.test.mjs's thin-plan fixture raised from 1.6x to 1.8x spending (with the final year exact it simulated 85.0% on its 200 paths, exactly at the 'auto' threshold, where it had read 84.0% - a fixture on the edge, not a thin plan); every audit-s126.mjs mode's ran line now records the final year it ran, placed before bridgeRead (the forty-fourth review, 10:28 UK: after it, smoke.sh's end-anchored check failed - the bugs list), and read-f1v2.mjs and read-bridgequad.mjs refuse an arm run exact (their predictions ran it averaged; both readers' output on their real logs unchanged); 7k, O19, 7e and 8f updated | decision: maintainer 25 Sep 09:36 UK |
...
|---|---|---|---|
| 25 Sep 22:40 UK, the forty-eighth review | M12's falsifier ("worse by more than two paired se") and M15 v2's "nothing if no arm gains beyond noise" use the two-se reading the regimen retired (RULES.md section 8) | Claude | each one's prediction, before it is registered |
| 25 Sep 22:45 UK, the forty-eighth review's MINOR 4 | fair-gate.mjs (the log gate's header) and .githooks/pre-commit date CI's red from 25 Sep 06:00 UK; it ran from 24 Sep 14:00 UK (the bugs list). CLOSED 25 Sep 22:59 UK: both corrected in 4c42eda under the maintainer's unlock | Claude | done |
| 26 Sep 06:41 UK, the fiftieth review's BLOCKING 2 | .githooks/pre-commit l.28 dates the maintainer's answers "25 Sep 22:47 UK"; they were recorded at 22:45 UK (21:45:54Z). The file is locked | Claude | the next unlock |
| 26 Sep 01:52 UK, the forty-ninth review's BACKLOG 6 | research/tests/solver-final.test.mjs l.28 and research/audit/invariants.mjs l.27 draw random cases from (seed * 1103515245 + 12345) & 0x7fffffff in doubles, which loses its low bits and cycles every 10,466 values (the bugs list, 26 Sep). They pick test and audit cases, not a result's paths, so no figure rests on them; their coverage is narrower than it looks | Claude | before Phase 4's full read |
...
|---|---|---|---|---|
| 0-7k | **Finished 23-25 Sep: moved to PLAN-HISTORY.md, "The schedule's finished rows"** (25 Sep 22:01 UK, the regimen's sweep; each row's outcome is in its own text there, and the ledger) | - | - | done |
...
| 7e | **Now (25 Sep 22:55 UK): read by the regimen's exact rule with a second look (reduce-7e.mjs; the ledger's 21:05 UK row), look 2 at 8,000 paths on bridge 4, wealth x0.5, S130 and S128 and 3,000 on the rest, the pooled floor by fixed effect (the maintainer, 22:45 UK); check 6 measured, about 3 to 6% at 16 points (results-readertime.txt); the reader's reference on the solver's growth convention (the bugs list); registered once a review passes on the fixed version (the forty-seventh failed, 21:56 UK, and the forty-eighth, 22:40 UK: the path count not put to the maintainer, now answered) and CI is green, then launched through the launcher.** RE-ORDERED 25 Sep 07:29 UK (maintainer: "I approve all your recommendations. Proceed."): the boundary-plus-residual reader is built and tested FIRST, against off, F1 v1 and F1 v2 (drafts/reader-design.md); F2 is HELD and built only if the reader fails; the thin S124, S128 and S130 join the panel so O5 is answered in the same run; survival, estate and the cut term are reported for every arm, beside the gap.** The earlier text: **F2: build and test** (the maintainer's decision, 13:51 UK): the coverage coordinate in bridge years (`f2-design.md`), built with its own planted tests in the run gaps (no cores); then off against F1 v1 against F1 v2 against F2 (v1 added 18:21 UK: the two F1 tests ran in different settings, so v1 against v2 has never been measured in one), paired, on 7c's cases on the step-6 defaults in the mixture, with the solve times; its prediction first. **Approved if** its results are positive and it costs at most 20% more run time, both defined in the prediction before the run. The grid is named there too: its reads judged at 16 points as 7c's, then at the product's 30 points on S126, bridge 6 and S366 before any approval; the time bar judged at 30 points (the fifteenth review) | 7c; 7g (read 25 Sep: its item 5 held, so the maintainer's 20:15 UK condition - 7e gains the outside review's boundary-plus-residual reader as a candidate arm if 7g's final year holds - is met); 7i (read 25 Sep 04:28 UK: the misread stays with finer averaging, so the reader is built and joins 7e - a fourth arm now F2 is held; the template-integral averaging is not built as a bridge fix); O22 traced before 7e's prediction is registered; S360 also run at 5 and 15 return points with the reader (7j: 15 points gain on S360, every decided path failing in the last two pre-access years though the moves differ from year 0; whether the reader closes the gap is the first evidence for 7l, the template-integral averaging back as a candidate); the dislike-of-cuts setting held at S126's landed lambda, 0.0223606797749979, as 7c ran, so 7e stays comparable with 7c (O15's default, decided in the build gaps, is carried by the combined run, 8f); the reader built, its own checks passed (drafts/reader-design.md; its reference chance built and checked 25 Sep, research/tests/reader.test.mjs; **built into the solver and checks 2 to 4 passed 25 Sep 11:13 UK, results-reader-checks.txt; check 6 measured 25 Sep 20:37-21:14 UK, results-readertime.txt**) and its smoke line (**added 25 Sep 11:14 UK under the maintainer's unlock: the bridge7e mode**) - F2 is held and built only if the reader fails (the maintainer 25 Sep 07:29 UK); 7k decided; every 7e arm runs with the final year exact (O19: carried forward, no harm; the product default since 25 Sep 09:36 UK, 7k); 7j read first (O22); the risk-above setting held explicitly and identically in every arm (riskAbove true, as 7c ran), with three worlds throughout - the product default is 'auto' since 25 Sep 06:31 UK, which decides per solve from the arm's own simulated survival and so could allow the tier above in one arm and not another (the outside reviewer's second reply: keep tier eligibility and the world count fixed across the arms); audit-s126.mjs's f1v2 and quad modes leave riskAbove unset and now run under 'auto' (7c and 7i ran under 'on'); the reader's design, its planted checks and the panel the reviewer asked for are drafted in drafts/reader-design.md (outside-review-reply-2.md) | re-estimated 25 Sep 22:55 UK from measured cells under load (rule 9; the forty-eighth review's MINOR 5): wave 1 about 2.7 to 3 h (7c's four-at-a-time pairs took 823 s on average, results-f1v2.txt, so four arms about 1,650 s a case and six cases a process, plus the exact final year and the reader's 3 to 6%); look 2 runs the cases look 1 leaves open - 11 expected, plus S162, S172 or S168 if open - about 3.8 core-hours (7 at 3,000 paths, 460 s an arm, and the four at 8,000, about 920 s an arm, two arms each; results-look2time.txt), beside wave 2's controls and S360 (about 0.6 core-hours at 16 points); the 30-point reads and the time bar at 30 points NOT MEASURED (M14b's 30-point households took about 12 minutes each with 3,000 paths, m14b-down, so p30's six solves about 1.2 h in one process and the time bar's twelve about 2.5 h alone - indicative only); in all about 6 to 7.5 h. The 22:01 UK text: wave 1 about 2 to 2.5 h from check 6's quiet-box solves; wave 2 and look 2 about 1 to 1.5 h. The earlier text: the test ~2.7 h at 16 points for four arms - off, v1, v2 and the reader (7c's part 0 took 81 min for a pair, so ~160 min) - plus the thin S124, S128 and S130 (not yet estimated) and S360 at 5 and 15 points with the reader (~20 min: 7i's S360 solves took 317 s and 852 s), before the reader's own cost; every arm holds riskAbove true, which gives the tiersAbove 1 read-f1v2.mjs wants - but read-f1v2.mjs and read-bridgequad.mjs refuse an arm run with the final year exact since 25 Sep (their predictions ran it averaged; the forty-fourth review), and every 7e arm runs it exact, so 7e's reducer names finalIntegral true in its own gate rather than reusing read-f1v2.mjs's, plus the 30-point reads on S126, bridge 6 and S366 and the time bar at 30 (not yet estimated) | after 7j, 7k and the reader's build |
| 7l | **Finer earlier-year averaging, back as a candidate** (7j's registered consequence, 25 Sep 07:54 UK: its falsifier fired): first the evidence already due - 7e's S360 arms at 5 and 15 points with the reader - and the outside reviewer's split on S360 from year 0 (the same continuation table averaged over 5 and over 15 points, then the two tables under one rule), each reported on the full score, since on S360 15 points traded end wealth for survival (results-o22-detail.txt, not registered); then the template-integral averaging (the outside review's first proposal: a known cliff template integrated exactly and the return points applied only to its residual; summarised in outside-review-findings.md section 2) built and tested, its prediction first, if the gain holds with the reader. Claude's sequencing; its place in the order is put to the maintainer, not decided. **The regimen's value-of-information screen (25 Sep 22:01 UK): after Phase 4, unless 7e's S360 arms show a general effect** | 7j; 7e's S360 arms | the split not yet estimated (four solves of S360 at ~5 to 15 min each, 7i); the build not sized | after 7e |
| 8 | **K6 slider spread** (in c, R5), with **K7 read off K4's, K5's and K6's sweeps** (no run of its own). **Held: K5 stage 1 found no matching point (14:38 UK); after option A (16:35 UK) it sweeps around the dislike-of-cuts reference (O15)** **O15 decided 25 Sep 20:31 UK: the reference is c = 0.001; K6 sweeps c 0.0001 to 0.01 in the product configuration, at each household's own tier and held at Medium, on 3,000 held-out paths of seed 7012 (a seed of its own: 7011 is 7e's, the seed registry; the forty-seventh review), its prediction registered first (drafts/o15-reference.md)** | O15 | ~1.5 h | after O15 |
| 8b | **M15 v2** (waits for 7m: its derivation takes 1 + R as the mean): build in K5's run gaps (no cores), after Q12 is put to the mathematician; probe (4 arms, 8 households + 3 at 40% gain, ~2 h). **Held: its probe runs at K5's point, which stage 1 did not find (14:38 UK); after option A (16:35 UK) it runs at the dislike-of-cuts reference (O15)** **(c = 0.001, decided 25 Sep 20:31 UK)** | O15 | ~2 h | after O15 |
| 8d | **The pitfall sweep** (C1-C5, C7 and C8 above, and the register's O2, O4, O5 and O9; O11 moved after Phase 4 and O5 answered in 7e, the maintainer 25 Sep 07:29 UK): the S126 fix, then the same pattern hunted, tested and fixed (an edit to audit-s126.mjs now re-runs the launcher's smoke run). **Waits for 7e and re-tests with the bridge fix chosen there; it runs before the combined no-harm run (8f), the maintainer 07:50 UK** (rule 8: 7e chooses among off, F1 v1, F1 v2 and the boundary-plus-residual reader, F2 held, (7i, 25 Sep 04:28 UK: the misread is the read, not the averaging) - off is the default since 19:25 UK; the fifteenth review, 14:17 UK). **Gates Phase 4** | 7e (the bridge fix chosen: off, v1, v2, or the reader; F2 only if the reader fails); if 7e carries the reader forward, 7o (its replication on seed 7004) or the maintainer's explicit acceptance of a grade-B default, before 8d re-tests with it | ~4-6 h, in run gaps | after 7e; not dated until the reader's build is sized |
| 8f | **The combined no-harm run (new, the maintainer 25 Sep 07:29 UK):** every new default together - 'auto' at 85%, the exact final year (DECIDED 25 Sep 09:36 UK; since solvePlan now runs it by default, the previous-baseline arm passes `finalIntegral: false` explicitly), the bridge fix 7e chooses, the dislike-of-cuts setting (O15) and five worlds only if O21 has adopted them by then - against the previous baseline, paired, on a broad panel of ordinary library households chosen by a rule that never looks at the solver, with survival, estate and the cut term reported per household; its decision rule is the regimen's (RULES.md section 8; adopted by the maintainer 25 Sep 20:47 UK, written into this row 22:01 UK): survival primary, Holm across the households, the margins (0.25 points where the previous baseline survives 95% or more, 0.5 below), a pooled mean with its floor of -0.1 points (random-effects as the regimen has it, unless 8f's prediction takes a floor a gain cannot widen: a random-effects interval widens with any spread, gains included, so on a panel where a few households are expected to gain a lot it can fire with no household losing - 7e's forty-seventh review, BLOCKING 1; over 7e's pool one case gaining 3 points fires it in 6 to 13% of simulated runs against about 1% for a fixed-effect floor, with the same power against a loss on every case, results-pooled-floor.txt (rebuilt 26 Sep with a sound generator; the first version's cycled, the forty-ninth review), grade C; a fixed-effect floor catches a small loss spread over three to six households 8 to 10 points less often (results-pooled-floor.txt, Holm across 24), which 8f's prediction must weigh; the same pattern carried here by the forty-eighth review, MINOR 3), spending, cuts and estate descriptive, the risk-based arm and year-on-year falls reported beside it (8i), set in its prediction, registered first (a plain "no household beyond two se" would fire by chance about half the time at 30 households, the thirty-ninth review); then R8 re-derived. If it shows problems, couples (O11) and the end-of-plan reader (the outside reviewer's joint bridge-and-terminal template, outside-review-reply-2.md) come back before Phase 4. Why: each default was tested alone and mostly on households chosen because they misbehaved. **Decided by the maintainer, 07:50 UK ("I approve your new recommendations"):** the pitfall sweep (8d) runs first, since it can change code this run should cover, and O21 first, so an adopted five-world setting is in it | 7e, 7o (or the maintainer's acceptance) if the reader is carried forward, 7k, O15, O21, 8d | sized when the panel is fixed | after 8d |
| 8e | **The retro fair-test audit**: every result the current defaults rest on through `fair-test.mjs`, code identity from git history; a failure is marked provisional and re-run. **The regimen (25 Sep 22:01 UK): every paired comparison a default rests on re-read from its records with the exact rule (stats.mjs: exact McNemar, Holm, the margins), with a list of the conclusions that change; and one claim-linting pass over the PLAN-HISTORY claims decisions still rest on, each given its evidence grade.** Also re-run into results files the two analyses the ledger rests on that are in none: M14's path-outcome split and the M15 mechanism diagnostic. **And O14's re-read** (the eighteenth review): every solver-arm depth, total cut and raise total made before 24 Sep 15:19 UK, re-read from the per-path records where they exist (`reduce-k5.mjs records` shows how; the raw records are archived) or re-run where they do not - flex-tiers has no record files for its 41 households | - | the audit and the records re-read need no cores (~1 h, not re-measured); the re-runs are not estimated until their list is drawn up | before Phase 4 |
| 7m | **The growth convention settled** (the outside review's Finding 3; the regimen, 25 Sep 22:01 UK): the solver grows a pot by exp(ln(1 + R) + V z), so R is the median; settled from the capital-market source, and R2's and M15 v2's derivations re-derived under it (the reader's reference already fixed, the bugs list 25 Sep) | - | no cores | while 7e runs |
| 7p | **The year-0 rollout** (the outside review's Finding 7, placed 26 Sep after the Road to Phase 4 update showed it had no row): a one-year rollout from a simulation Part C already plans, a check on the table's first move; registered first with the regimen's fields | 7e | not yet sized | after 7e, before Phase 4 |
| 7n | **The table-gap decomposition** (the review's Finding 2): the table's optimism split into grid, quadrature and information parts, four arms, registered first with the regimen's fields | 7e | not yet sized | after 7e |
| 7o | **7e's replication on seed 7004** (the forty-eighth review, MINOR 7; the regimen's replication rule, RULES.md section 8 item 8: one 7e run is grade B): if 7e carries the reader forward, off against the reader on the cases it decided, 3,000 paths of seed 7004 (registered to replication), its prediction registered first with a Seeds field; the reader becomes solvePlan's default only after it, unless the maintainer accepts the grade-B risk explicitly | 7e carried forward | not sized: from 7e's own measured cells | after 7e, before 8d |
| 8g | **Perturbed engines and the first error-budget entry** (the review's Finding 5 and section 14): the survival change under 2c.1's existing perturbed engines on the twelve step-2 households (not measured: sized from a measured cell before launch, rule 9; the forty-eighth review), then the two-factor, fat-tailed-with-inflation-shocks and bootstrap engines built for Phase 4's condition 4 | 7e | not measured; then the builds (not sized) | before Phase 4 |
| 8h | **The optimality ceiling** (the review's Finding 6): the hindsight bound on failing paths, then the penalised bound and the threshold benchmark; its gap sets when grid and cliff work stops | 7e | not yet sized | after 7e; before Phase 4 only if cliff work is still open |
| 8i | **The risk-based comparison arm and year-on-year falls** in the harness (the review's Findings 4 and 5), reported in 8f and Phase 4 | - | build, no cores | before 8f |
| 8j | **The scorecard**: each prediction item's credence against its outcome, the Brier score per test and in all, a reliability table; 7e is its first entry. BUILT 26 Sep 07:10 UK (scorecard.mjs; the ledger); re-run after each reducer's output is saved, each new test added to its TESTS list | - | build, no cores | done; runs when 7e is read |
| 8k | **The pension inheritance-tax fixture** (the review's Finding 8): one synthetic household that also settles M8 | - | build, no cores | before estate copy ships |
| 9b | **Historical calibration** (the review's Finding 5): reported survival against history, Brier score and a reliability curve, arm S no worse than arm A; ungated | Phase 4's panel | not yet sized | beside Phase 4 |
| 9 | **Phase 4**, with its bundled extras (below); M23 decided (A, built): arm A runs with `raiseCap: 1.1`. **The regimen (25 Sep 22:01 UK): gate 4 keeps its margins, read with exact intervals; condition 4 gains the new engines (8g); the diagnostics gain the risk-based arm and year-on-year falls (8i) and the historical calibration (9b).** **Waits for the pitfall sweep (8d), the retro fair-test audit (8e) and the combined no-harm run (8f)** | 8, M23, 8f, O21 (its own gate), 7m (R2 and R8 re-derived on the growth convention), 7o (if the reader is carried forward), 7p (the year-0 rollout) | ~6 h | after 8d, 8e and 8f; not dated until the reader's build is sized |
...
S126's replication, root cause, fix options and F1's build, test and result are finished and in PLAN-HISTORY.md ("S126's dead corner (#106)", moved 25 Sep 22:01 UK); the bridge fix is chosen at 7e.
...
FALSIFIED IF the exact rule (RULES.md section 8: Holm across the households, the margins) finds harm for any arm against
today on any household, or the ladder's solve on a
...
1. **Survival:** arm S higher across Panel H: the random-effects pooled change's 95% interval above zero, with a clear sign test (the regimen, RULES.md section 8, 25 Sep 22:01 UK).
2. **Not paid for in spending:** spending delivered not lower on average by more than 1%.
3. **No household badly worse:** none shows harm at a 1-point survival margin by the exact rule (Holm across the panel,
   25 Sep 22:01 UK), and none loses more than 5% of spending delivered. A household the rule leaves inconclusive does not
   pass silently: it is extended to more paths (RULES.md section 8 item 3), and one still inconclusive is reported with
   its bound to the maintainer, who decides before the gate is read (25 Sep 22:45 UK, the forty-eighth review); Phase 4's
   prediction (tier 2) sets the final form.
4. **Robust:** the sign holds on the historical backtest, on all three perturbed engines of Phase 2c and on the regimen's
   three new ones (8g: two-factor, fat tails with inflation shocks, the bootstrap).
