...
| 8d | **The pitfall sweep** (C1-C5, C7 and C8 above, and the register's O2, O4, O5, O9 and O11): the S126 fix, then the same pattern hunted, tested and fixed (an edit to audit-s126.mjs now re-runs the launcher's smoke run). **Waits for F2's test (7e) and re-tests with the bridge fix chosen there** (rule 8: 7e chooses among off, F1 v1, F1 v2, F2 and, if 7i calls for it, the boundary-plus-residual reader - off is the default since 19:25 UK; the fifteenth review, 14:17 UK). **Gates Phase 4** | 7e (the bridge fix chosen: off, v1, v2, F2 or the reader) | ~4-6 h, in run gaps | after 7e; not dated until F2's build is sized |
