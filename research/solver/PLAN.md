...
| O5 | With F1 on, the thin in-class households still hold the pension below its tier most of the plan (S124 39.1, S128 29.7, S130 36.9 years; bridge 4 42.0), and on wealth x0.5 it rose (36.8 -> 39.4): F1's item 3 | 24 Sep, F1 test (x0.5 added 10:02 UK; found by the plan-auditor, 09:45 UK) | Claude | the pitfall sweep (8d), before Phase 4 (left open by the F1 write-up, 12:24 UK: results-f1-verdict.txt) | open |
...
| O9 | With F1 on, the in-class tables turn optimistic: S130 +5.3 (outside F1's item 1, +/-5) and S128 +3.2 - the risk already named for F1 | 24 Sep 09:45 UK, plan-auditor | Claude | the pitfall sweep (8d), before Phase 4 (left open by the F1 write-up, 12:24 UK: results-f1-verdict.txt) | open |
...
| O13 | K5 stage 1: the median household cuts at most 37% of the guardrails' total even at c = 0.0001 and raises about three times as much; per household, eleven deliver more spending for survival at least as good, but S070 cuts more at every point and delivers 3.5% to 9.2% less spending for more survival (results-k5-stage1.txt). Why is NOT CHECKED: the likely reading is that a cut earns something in the solver's objective only when it raises survival | 24 Sep 14:38 UK, K5 stage 1 | Claude | the maintainer's decision on Phase 4's fairness condition (option B would check it), before Phase 4 | open |
...
| **dislike of spending cuts, 0 to 100%** | how much a trim below target hurts | the trim penalty lambda - today found by the landing per household, ranging 0.005 to 2 (a 400x span) | **yes - a user level, agreed 23 Sep.** NOT BUILT as a level. Its DEFAULT was to be matched to how much the guardrails cut (K5); **K5 stage 1 found no such point (14:54 UK) - the default's basis is the maintainer's decision (the K5 section)**; K6 calibrates its spread around it |
...
| **trim curve** | how the cost of a trim grows with its depth, between target and floor | the shortfall exponent, 2 | **no - a fixed default; adjustable in a later build.** Its default was to be FITTED to the guardrails' shape (K5); **stage 1 matched the depth only where the total cut is a sixth of theirs - pending the maintainer's K5 decision (14:54 UK)** |
...
| R7 | **Phase 4's configuration after step 6.** | - | Arm S: floor fix on, cap 1.1, minimum pot 1 year, estate as today, risk above the tier ~~OFF (an opt-in is not a default)~~ **ON, as the default now is (maintainer 24 Sep; PROVISIONAL until M14b) - nothing changes for library households at the top tier (M21), but it decides the Medium-tier diagnostic** (corrected 24 Sep 09:48 UK, plan-auditor), GIA tier off, K5's c and exponent. The panel is LANDED at ~85% for arm A (decided): each household's target is set so arm A survives 85% +/- 2 on seed 7005 with the minimum pot on (the panel is now drawn after the defaults, so it is landed with them). Added diagnostic: **arm S with pension and ISA held at Medium** (M21: the library never tests a cautious user; this is also the only place risk above the tier can show anything, since at High there is no tier above). |
| R8 | **Phase 4's survival prediction, re-derived.** | The floor fix adds 0.2-0.9 points on thin households (M17); the tiers' edge is concentrated where the lower limit binds (R2); the landed panel sits where decisions are not near-ties. Against that, K5's matched cutting is ~6x today's, which moves survival UP (cuts are the solver's other protective lever). **Superseded 14:54 UK: K5 stage 1 found no matching point - the solver cuts 0.16 to 0.79 at the median, at most about twice today's 0.40 (results-k5-stage1.txt); this support is gone, and R8 is re-derived after the maintainer's K5 decision** | Prediction strengthened: arm S wins survival on at least 30 of 40, and the no-tiers diagnostic keeps a minority, as before. Spending: see R3, which is the condition most at risk, not survival. |
...
| 7 | **K5 guardrail matching** - **stage 1 FINISHED ~14:30 UK and FALSIFIED** (`results-k5-stage1.txt`: no setting reaches the guardrails' total cut; stages 2, 3 and 3b held for the maintainer's decision, 14:38 UK). As planned: stage 1 from 06:38 UK (288 cells at ~6.5 min each, four at a time: **~8 h, not 4.5**; 64 done at 08:40, 168 at 11:11 UK (k5.log): about 41 an hour, so **finish ~14:05 UK**, not ~18:00), judged against the corrected fold target (08:39); stage 2 now expected (R4 re-derived); stage 3 on the 41 **in the mixture**; stage 3b, the twelve at Medium with risk above (12 cells plus the guardrails at Medium; ~35 min for the twelve at four at a time - measured: K5's 212 cells, median 5.7 min in the fold, and the mixture ~2.06x the fold on bridge-41 (medians 1.32 against 0.64 min), so ~12 min a cell; re-estimated from its first cells) | 6 | stage 1 ~8 h (done); the rest held | stage 1 done; the rest held |
| 7b | **M14b** (`batch-m14b.sh`, 24 cells, **in the mixture**, ~2 h; registered prediction `predictions/m14b.md`): risk above the tier re-checked under the step-6 defaults; decides whether "on in every plan" stands or falls back to 'auto' (thin plans, no-worse guard). Also C8's check for risk above | 7 stage 1 | ~2 h | **running: the batch from 14:37 UK (runs.log), so ~16:40 UK** |
| 7c | **The F1 v2 test** (approved 12:42 UK): off against v2, paired, on the step-6 defaults **in the mixture** - the twelve S126 variants, S120-S130, S360, S366 and S370, and the cost case (bridge 4 with a 30k one-off cost in its year 2, added before any run at the maintainer's request, 14:05 UK); its prediction (`predictions/f1v2-test.md`) written and pushed before it launches. Settles O2, O4 and O9 or leaves them open; whether F1 becomes the default waits for it and for 8d. It solves at 16 points, as the F1 test did; the product solves at 30, so before any F1 default v2 is also checked at 30 points on S126, bridge 6 and S366 (the fourteenth review) | 7 stage 1; 7b, since M14b may change the risk-above default this test runs on (rule 8; the fourteenth review); F1 v2 built; the f1v2 smoke line (in, 13:44 UK) | ~1 h (the F1 test took about 5 min a case in the fold, x2.06 for the mixture - the stage-3b estimate's ratio - so ~10 min a case; 21 cases split four ways put six in part 0, ~63 min), re-estimated from its first cases | after M14b |
| 7d | **Couples in a bridge (O11)**, then **the thin households' lower tier (O5)**: is either a second misread? Each measured paired, with its prediction first; F2 is now its own step, 7e | 7e (O5 and O11 depend on the bridge fix chosen there: rule 8; the sixteenth review) | not estimated yet | after 7e |
| 7e | **F2: build and test** (the maintainer's decision, 13:51 UK): the coverage coordinate in bridge years (`f2-design.md`), built with its own planted tests in the run gaps (no cores); then off against F1 v2 against F2, paired, on 7c's cases on the step-6 defaults in the mixture, with the solve times; its prediction first. **Approved if** its results are positive and it costs at most 20% more run time, both defined in the prediction before the run. The grid is named there too: its reads judged at 16 points as 7c's, then at the product's 30 points on S126, bridge 6 and S366 before any approval; the time bar judged at 30 points (the fifteenth review) | 7c; F2 built and smoke-tested | the build not estimated yet (it changes the grid's coordinates in bridge years); the test ~1.5 h at 16 points (three arms where 7c has two, from 7c's own ~1 h), plus the 30-point reads on S126, bridge 6 and S366 and the time bar at 30 (not yet estimated) | after 7c |
| 8 | **K6 slider spread** (in c, R5), with **K7 read off K4's, K5's and K6's sweeps** (no run of its own). **Held: K5 stage 1 found no matching point (14:38 UK)** | 7 | ~1.5 h | held |
| 8b | **M15 v2**: build in K5's run gaps (no cores), after Q12 is put to the mathematician; probe (4 arms, 8 households + 3 at 40% gain, ~2 h). **Held: its probe runs at K5's point, which stage 1 did not find (14:38 UK)** | 7 | ~2 h | held |
| 8d | **The pitfall sweep** (C1-C5, C7 and C8 above, and the register's O2, O4, O5, O9 and O11): the S126 fix, then the same pattern hunted, tested and fixed (an edit to audit-s126.mjs now re-runs the launcher's smoke run). **Waits for F2's test (7e) and re-tests with the bridge fix chosen there** (rule 8: F2 may replace F1 v2; the fifteenth review, 14:17 UK). **Gates Phase 4** | 7e (the bridge fix chosen: F1 v2 or F2) | ~4-6 h, in run gaps | after 7e; not dated until F2's build is sized |
| 8e | **The retro fair-test audit**: every result the current defaults rest on through `fair-test.mjs`, code identity from git history; a failure is marked provisional and re-run. Also re-run into results files the two analyses the ledger rests on that are in none: M14's path-outcome split and the M15 mechanism diagnostic | - | no cores, ~1 h | before Phase 4 |
| 9 | **Phase 4**, with its bundled extras (below); M23 decided (A, built): arm A runs with `raiseCap: 1.1`. **Waits for the pitfall sweep (8d) and the retro fair-test audit (8e)** | 8, M23 | ~6 h | after 8d and 8e; not dated until F2's build is sized (14:17 UK) |
...
then re-tests with the chosen fix.)
...
**Gate:** Phase 4 does not start until C1-C5, C7 and C8, and the register's O2, O4, O5, O9 and O11, are each cleared or fixed, and re-tested with the fix (the bridge fix being the one chosen at 7e) (C7 added 24 Sep 09:48 UK after the plan-auditor's review; C6 alone is deferred, to the couples work).
...
**STAGE 1 - THE RESULT (read 24 Sep 14:32 UK by `reduce-k5.mjs` over the 288 cells; `results-k5-stage1.txt`): FALSIFIED.**
No point of the grid brings the solver's median total cut near the guardrails' 2.14 years: it runs from 0.16 (c 0.03,
exponent 1.5) to 0.79 (c 0.0001) - at most 37% of the target. The fair-test gate passed with row 28 as registered
(ACCEPTED in the prediction during the run, before any cell was read: b370285 09:30 UK, final wording e3ec5eb 10:34 UK); the cells' code is established by hand (one snapshot of 31a1b52, 7824be314568).
1. **The total cut rises as c falls, at every exponent (held), but never reaches the target (missed).** Each step of a
   factor of three in c adds only about 11-15% at the low end (0.69 -> 0.79 at exponent 1.5, 0.70 -> 0.78 at exponent 2),
   so reaching 2.14 would take about nine more steps (c near 1e-8) - no dislike of cuts at all.
2. **Depth: held where it was predicted** - within 0.03 of 0.892 at exponent 3 and 4 with c 0.01 and 0.03 (0.881 to
   0.899) - but there the total is 0.31 to 0.38, a sixth of the target.
3. **Survival** at or above the guardrails' on 11 or 12 of 12 at every point (previewed, not tested).
Raises run 3.17 to 3.33 years against the guardrails' 1.07 at every point. **The medians hide the households**
(corrected 14:54 UK, the seventeenth review; results-k5-stage1.txt, per household):

    S054 S100 S112 S126 S162 S206 S252 S390 S354   cut less than the guardrails at (nearly) every point; deliver 5.9% to
                                                    18.4% more spending; survival equal or up to 7.2 points higher
    S184 S330                                       cut MORE at 20 of 24 points, but raise enough to deliver 3.9% to 9.7%
                                                    more spending; survival 1.9 to 11.2 points higher
    S070                                            cuts MORE at all 24 points (3.02 to 5.49 years against 2.18) and
                                                    delivers 3.5% to 9.2% LESS spending, for 1.7 to 12.6 points more survival

So on eleven households the solver spends more and survives at least as well, and on S070 its survival edge is
bought with spending - exactly what the fairness condition was written to catch. **Why is NOT CHECKED** (O13): the
likeliest reading is that a cut pays in the solver's objective only when it raises survival - the comfortable
households gain nothing from cutting, while a thin one like S070 gains a great deal - whereas the guardrails cut by rule.

**Decision for the maintainer (put 14:38 UK; restated 14:54 UK against gate 4 as written, the seventeenth review):**
stages 2, 3 and 3b, K6 and M15 v2 all need K5's point and are held. No single setting matches the guardrails' cutting,
and the mismatch runs both ways by household. Gate 4 already carries the spending test: condition 2, spending delivered
not lower on average by more than 1%; condition 3, no household more than 1 point of survival or 5% of spending worse.
(A) **Recommended:** drop cut-matching as Phase 4's precondition and let gate 4's conditions 2 and 3 carry fairness, as
written: on these twelve at stage 1's settings condition 2 passes at every point (eleven deliver more), and condition 3
is decided by households like S070, which delivers 3.5% to 9.2% less depending on the point - so the dislike-of-cuts
default is chosen with condition 3 in view (at the points where S070 is within 5%), not by matching the guardrails' total.
(B) Before deciding, a diagnostic of why S070 cuts more and the others less (two households at c = 0.0001 and one
middle point; its own prediction and the launcher first; about 15 minutes at stage 1's pace of 363-468 s a cell).
(C) Extend the grid to lower c: not recommended - it would take c near 1e-8, and it would push S070 further the wrong way.
...
spending than the guardrails and S070 less (3.5% to 9.2%), so gate 4's condition 3 is the one at risk. This prediction
and R8 are re-derived after the maintainer's K5 decision, before Phase 4 runs.** The earlier text follows.
...
|---|---|---|
| trim curve exponent, 2 | one deep cut or several shallow ones | **was to be fitted in K5; pending the maintainer's K5 decision (stage 1 FALSIFIED)** |
| lambda (dislike of cuts) | how much is cut | **a user slider; its default's basis pending the maintainer's K5 decision (stage 1 FALSIFIED), spread in K6** |
