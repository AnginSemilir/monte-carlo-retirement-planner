...
| then | **E4's measurement**, plus **the solve/forward split**, ~25 min | Two twenty-minute questions in one slot. E4's: time four separate arrays against one interleaved at the real size (`research/solver/e4-measure.mjs`); negligible difference and E4 is dead for twenty minutes rather than a morning. **The split's, added 22 Sep and the more important of the two**: every phase of Part E speeds up the SOLVE and none of them touches the forward passes. If those are half a landing, E3's measured 30.2% of cell work is 30.2% of half, and the cheapest remaining win is the never-swept 5,400 search paths (task #108) rather than E3. `research/solver/split.mjs` settles it directly instead of by differencing two runs. **Both need a quiet box** - a cache benchmark run against four busy cores measures contention, not layout, and `e4-measure.mjs` refuses if the experiment lock is held. |
...
| **after Phase 7** | **E2**, split the cells across cores | **Moved out of this sequence 22 Sep: it could never have run here.** Its own entry says it is designed with Phase 7's worker plumbing, and Phase 7 is in Part C, which is gated on Phase 4's decision - so listing E2 before Phase 4 asked it to wait for something that came after it. Concurrency is also the one place a passing test is not proof: a race can pass a hundred times and fail on the hundred-and-first, so bit-equality here is evidence rather than a guarantee. |
