...
- **`runFixedPath` called `world(...)` with an undefined `act`** (the M15 edit replaced both `world` calls in
  ...
  other audit-*.mjs scripts and experiment.mjs's select/run/perturb/reduce are not in it - one is added before a batch
  that uses it runs on edited code (smoke.sh's header; corrected 11:11 UK after the plan-auditor, 11:05 UK).
...
- **The PreToolUse hook judged whole command lines, not their parts** (found 24 Sep ~09:55 UK when an audit run got through):
  ...
  and against the committed whole-line hook the new one refuses everything it refused plus seven forms it let through
  (kill by a `pgrep -f` pattern, a `+branch` force push, `pkill -f` inside `bash -c`, a batch run directly, a launch after
  a syntax check, a launcher named in a comment, a launcher named in another part), and no longer reads `nice -n 5 git
  commit` as `git commit -n`. **Same pattern searched:** every rule in pre-tool.mjs: the hooksPath rule was whole-line
  ...
  towards refusing). **Still not seen (a guardrail, not a sandbox):** a program that runs a command itself (a node or
  python script calling a shell).
...
- **`audit-s126.mjs ids` read its grid size and path count from the wrong arguments** (found 24 Sep 09:45 UK by the
  ...
  and paths it was given (committed after the maintainer's unlock, 11:00 UK).
