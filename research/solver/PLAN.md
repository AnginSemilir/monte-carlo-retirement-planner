...
|---|---|---|---|
| 25 Sep 07:32 | (maintainer) **The road to Phase 4 re-ordered by likely gain** ("I approve all your recommendations. Proceed.", 07:32 UK, answering the morning's ranking and its holistic check): (1) F2 is held and the boundary-plus-residual reader is built and tested first; (2) the exact final year's default is decided after its timing run (7k), before 7e; (3) the order: 7j, 7k, the reader's build, 7e (with the thin households for O5; survival, estate and cuts reported), the dislike-of-cuts decision (O15) prepared in the build gaps, O21, the pitfall sweep (8d), a new combined no-harm run of every new default together on a broad ordinary panel (8f), then Phase 4; couples (O11) after Phase 4. Laid out for the maintainer in the doc "The road to Phase 4" (https://claude.ai/artifact/MKmfKXb3wk5Gdsh3XDSeF4) | 7e re-ordered (F2 held); 7d re-scoped; 7k and 8f added; 8d and Phase 4's row; O5, O11 and O15's gates | decision: maintainer 25 Sep 07:32 UK |
...
| O5 | With F1 on, the thin in-class households still hold the pension below its tier most of the plan (S124 39.1, S128 29.7, S130 36.9 years; bridge 4 42.0), and on wealth x0.5 it rose (36.8 -> 39.4): F1's item 3 | 24 Sep, F1 test (x0.5 added 10:02 UK; found by the plan-auditor, 09:45 UK) | Claude | 7e (the thin S124, S128 and S130 in its panel, the maintainer 25 Sep 07:32 UK; was the pitfall sweep, 8d), before Phase 4 (left open by the F1 write-up, 12:24 UK: results-f1-verdict.txt) | open |
...
| 8d | **The pitfall sweep** (C1-C5, C7 and C8 above, and the register's O2, O4, O5 and O9; O11 moved after Phase 4 and O5 answered in 7e, the maintainer 25 Sep 07:32 UK): the S126 fix, then the same pattern hunted, tested and fixed (an edit to audit-s126.mjs now re-runs the launcher's smoke run). **Waits for F2's test (7e) and re-tests with the bridge fix chosen there** (rule 8: 7e chooses among off, F1 v1, F1 v2, F2 and the boundary-plus-residual reader (7i, 25 Sep 04:28 UK: the misread is the read, not the averaging) - off is the default since 19:25 UK; the fifteenth review, 14:17 UK). **Gates Phase 4** | 7e (the bridge fix chosen: off, v1, v2, F2, or the reader) | ~4-6 h, in run gaps | after 7e; not dated until the reader's build is sized |
| 8f | **The combined no-harm run (new, the maintainer 25 Sep 07:32 UK):** every new default together - 'auto' at 85%, the exact final year if approved (7k), the bridge fix 7e chooses, the dislike-of-cuts setting (O15) and five worlds if O21 adopts them - against the previous baseline, paired, on a broad panel of ordinary library households chosen by a rule that never looks at the solver, with survival, estate and the cut term reported per household (no household losing beyond two se); prediction registered first; then R8 re-derived. Why: each default was tested alone and mostly on households chosen because they misbehaved | 7e, 7k, O15, O21, 8d | sized when the panel is fixed | after 8d |
...
| 9 | **Phase 4**, with its bundled extras (below); M23 decided (A, built): arm A runs with `raiseCap: 1.1`. **Waits for the pitfall sweep (8d), the retro fair-test audit (8e) and the combined no-harm run (8f)** | 8, M23, 8f | ~6 h | after 8d, 8e and 8f; not dated until the reader's build is sized |
