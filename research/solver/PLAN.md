...
| 25 Sep 11:13 | **The bridge reader built into the solver (`bridgeRead: 'reader'`), and its own checks 2 to 4 passed** (a build and its unit checks, not a test result; drafts/reader-design.md, the code committed as 2a0557a): the trap reads 0.40, not the unit template's 0.85; node reproduction over 7776 nodes of S126's 6 reader tables (two bridge years, three worlds), largest miss 5.6e-17; bit for bit from the last bridge year on (S126) and everywhere on S000 (no bridge); each check shown failing on a planted fault. The first run's planted fault for node reproduction (the residual dropped) had nothing to catch - on S126 the residual is at most 1.0e-6, the clamp, because every node either has c = S / p exactly or holds S near zero - so it was replaced before the second run by p taken as 1 on S126 and the residual dropped on a synthetic row where it carries value; both fail as they should. Callers re-tested on this code: reader.test.mjs 30 passed, solver-f1.test.mjs 19 passed, f1-default.test.mjs 5 passed, solver-step2.test.mjs 7 passed | 7e's reader dependency: built, checks 2 to 4 passed; check 6 (its run time, audit-s126.mjs readertime at 16 points) runs through the launcher next; the probes (check 5) are diagnostics inside 7e; its smoke line added with 7e's batch mode under the maintainer's unlock (the row above); drafts/reader-design.md records what the build chose (the reference mix: ISA, taxable account and cash by opening balance at the plan's tiers and each world's rates) | results: results-reader-checks.txt; fair-test: n/a (a build's unit checks, no arms compared); prediction: none (a build, not a test) |
