...
| O4 | F1 acted on the long-bridge controls it was predicted to leave alone: S360 (+3.90 +/- 0.64 survival; pension below tier 0 -> 17.4 years) and S366 (below tier 40.0 -> 28.2). And S366 simulates 98.9% with F1 off although item 5 called it "truly short" (floor coverage 0.77): the a\* test ignores growth over an 8-year bridge | 24 Sep, F1 test (S366 added 10:02 UK; found by the plan-auditor, 09:45 UK) | Claude | the F1 write-up | open |
| O5 | With F1 on, the thin in-class households still hold the pension below its tier most of the plan (S124 39.1, S128 29.7, S130 36.9 years; bridge 4 42.0), and on wealth x0.5 it rose (36.8 -> 39.4): F1's item 3 | 24 Sep, F1 test (x0.5 added 10:02 UK; found by the plan-auditor, 09:45 UK) | Claude | the F1 write-up (the lower limit binding, M22, or the misread?) | open |
...
| O8 | `audit-s126.mjs facts()` still computes a\* at the TARGET need, which the plan corrected to the floor need: `results-f1.txt` prints "class no" for share 0.95 and bridge 6, which are in class at the floor need (a\* 0.951 and 0.853, coverage 1.024) | 24 Sep 09:45 UK, plan-auditor | Claude | the F1 write-up (fix facts(); re-read the class column) | open |
| O9 | With F1 on, the in-class tables turn optimistic: S130 +5.3 (outside F1's item 1, +/-5) and S128 +3.2 - the risk already named for F1 | 24 Sep 09:45 UK, plan-auditor | Claude | the F1 write-up | open |
...
- **`runFixedPath` called `world(...)` with an undefined `act`** (the M15 edit replaced both `world` calls in
  ...
  And the launcher re-runs it only when the hashed code changes (code-id.mjs: engine.mjs, experiment.mjs, record.mjs,
  scenarios.mjs, src/solver), so an edit to an audit script or select-phase4.mjs ALONE does not re-run it (the
  plan-auditor, 11:24 UK). Until the hash covers every script a batch runs (proposed to the maintainer), after such an
  edit `bash research/solver/smoke.sh` is run by hand before the batch, and a mode is added to smoke.sh before a batch
  that uses it runs on edited code.
...
- **The PreToolUse hook judged whole command lines, not their parts** (found 24 Sep ~09:55 UK when an audit run got through):
  ...
  reduceFlex only reads result files and prints, like `reduce`, which was exempt already; the maintainer confirms it or
  has it reverted. **Same pattern searched:** every rule in pre-tool.mjs: the hooksPath rule was whole-line
  ...
  towards refusing). **Still not seen (a guardrail, not a sandbox; the plan-auditor, 11:24 UK):** a shell fed a
  here-document (`bash <<EOF`, then the commands, then `EOF`: the body is read as text, so a kill by pattern, a force
  push or a launch inside it passes); a launch of any script other than experiment.mjs, batch-*.sh and audit-*.mjs
  (select-phase4.mjs, couple-gate.mjs, bridge-gate.mjs, seedcheck.mjs run directly pass); a program that runs a command
  itself. And the unlock lasts until the maintainer's next message is DELIVERED - at the end of the turn they typed it
  in - so a message typed mid-turn does not relock that turn. **Proposed to the maintainer (each needs an unlock):**
  judge a here-document fed to a shell as commands; add those scripts to the launch rule; end the unlock when the
  maintainer's next message is queued, not delivered; and key the smoke stamp on every script a batch runs (below).
...
- **`audit-s126.mjs ids` read its grid size and path count from the wrong arguments** (found 24 Sep 09:45 UK by the
  ...
  settings check fires: 12 points, not 6, 11:33 UK). It runs only when the smoke run runs:
  an edit to audit-s126.mjs alone does not re-run it, so smoke by hand after one (above). The audit scripts' outputs do
...
| 8d | **The pitfall sweep** (C1-C5, C7 and C8 above): the S126 fix, then the same pattern hunted, tested and fixed; `smoke.sh` by hand after each edit to audit-s126.mjs (its edits do not trigger the launcher's smoke run). **Gates Phase 4** | S126 fix | ~4-6 h, in run gaps | Thu evening / Fri morning |
