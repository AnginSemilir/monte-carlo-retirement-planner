...
| O4 | F1 acted on the long-bridge controls it was predicted to leave alone: S360 (+3.90 +/- 0.64 survival; pension below tier 0 -> 17.4 years) and S366 (below tier 40.0 -> 28.2). And S366 simulates 98.9% with F1 off although item 5 called it "truly short" (floor coverage 0.77): the a\* test ignores growth over an 8-year bridge | 24 Sep, F1 test (S366 added 10:00 UK; found by the plan-auditor, 09:45 UK) | Claude | the F1 write-up | open |
| O5 | With F1 on, the thin in-class households still hold the pension below its tier most of the plan (S124 39.1, S128 29.7, S130 36.9 years; bridge 4 42.0), and on wealth x0.5 it rose (36.8 -> 39.4): F1's item 3 | 24 Sep, F1 test (x0.5 added 10:00 UK; found by the plan-auditor, 09:45 UK) | Claude | the F1 write-up (the lower limit binding, M22, or the misread?) | open |
...
- **The PreToolUse hook judged whole command lines, not their parts** (found 24 Sep ~09:55 UK when an audit run got through):
  ...
  same; a `-n` flag anywhere read as `git commit -n`. Each part of a command is now judged on its own; 53 tests prepared
  and passing against the live hook, but NOT YET IN THE REPO (the test file is locked until the maintainer unlocks it). And the first version's "ask" for the enforcement files was approved without the maintainer in auto mode; it is
  now a refusal only the maintainer's own message lifts ("unlock enforcement"). **Same pattern searched:** every rule in
  pre-tool.mjs: the hooksPath rule was whole-line too (a `--get` anywhere exempted the line) and is now per part; the
  lock rule was per part; the protected-file rule stays whole-command on purpose (it errs towards refusing). **Open
  (plan-auditor, 10:15 UK):** the rewrite anchors the kill, commit and push rules at the start of a part, so prefixed
  forms now pass (`FOO=1 git commit --no-verify`, `timeout 5 pkill -f x`, `xargs pkill -f`, `GIT_X=1 git push -f`) - to
  fix, with tests, when the maintainer unlocks the hook. The four enforcement files changed here await their approval.
...
- **`audit-s126.mjs ids` read its grid size and path count from the wrong arguments** (found 24 Sep 09:45 UK by the
  ...
  select-phase4.mjs) read their numbers inside each mode; `smoke.sh` now runs the `ids` mode and checks the settings it
  prints.
...
| 8d | **The pitfall sweep** (C1-C5, C7 and C8 above): the S126 fix, then the same pattern hunted, tested and fixed. **Gates Phase 4** | S126 fix | ~4-6 h, in run gaps | Thu evening / Fri morning |
...
- **Stage 1** (`batch-k5.sh`, 288 cells, ~4.5 h): the step-2 twelve, with the chosen defaults on (the M17
  ...
  **c, the cost of one year at the floor**, with lambda = c / 0.2^exponent, so "how much a floor year hurts" is
  ...
  - **the raise cap:** M23 (decided and committed 07:23 UK) makes arm A honour the cap. It changes the guardrails' cuts as well as
