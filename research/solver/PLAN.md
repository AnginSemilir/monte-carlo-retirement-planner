...
| 7l | **Finer earlier-year averaging, back as a candidate** (7j's registered consequence, 25 Sep 07:54 UK: its falsifier fired): first the evidence already due - 7e's S360 arms at 5 and 15 points with the reader - and the outside reviewer's split on S360 from year 0 (the same continuation table averaged over 5 and over 15 points, then the two tables under one rule), each reported on the full score, since on S360 15 points traded end wealth for survival (results-o22-detail.txt, not registered); then the template-integral averaging (outside-review-reply-2.md) built and tested, its prediction first, if the gain holds with the reader. Claude's sequencing; its place in the order is put to the maintainer, not decided | 7j; 7e's S360 arms | the split not yet estimated (four solves of S360 at ~5 to 15 min each, 7i); the build not sized | after 7e |
