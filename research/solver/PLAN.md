...
| O2 | F1 leaves long bridges misread: bridge 6 (table 53.3 against 99.3 simulated), S360 (4.3 against 44.1), S366 (3.0 against 99.2). **Diagnosed 12:41 UK** (results-f1-misses.txt): bridge 6 has an inheritance inside its bridge that F1's coverage test does not count; S360 is short on the no-growth test, so F1 stays off | 24 Sep, F1 test | Claude | the pitfall sweep (8d), before Phase 4 (left open by the F1 write-up, 12:24 UK: results-f1-verdict.txt) | open |
...
| O11 | F1 reads only the first person's bridge; 60 of the library's 210 couples have a bridge for at least one partner, and no couple was in the F1 test (results-f1-misses.txt) | 24 Sep 12:41 UK, the F1 write-up | Claude | before any F1 default, and before Phase 4 | open |
...
- **Money arriving later in the bridge is not counted.** F1's coverage test adds known costs but not known inflows. S366
  and S370 (library) and the bridge-6 variant each receive an inheritance inside the bridge, so the test calls them short,
  F1 stays off and the dead read stands - while the solver's own model counts the inheritance and they simulate at 99%.
...
- **Couples are untested:** F1 reads only the first person's bridge, and 60 of the library's 210 couples have a bridge (O11).
...
hard limit between grid nodes) may recur elsewhere. **Proposed to the maintainer (12:41 UK, not yet approved):** make
