...
#   look 2, in wave 2: the open cases at 3,000 paths of the same seed (the first 1,000 are wave 1's), off against the
#     reader, split over two processes when more than four are open; none open writes an empty look2.txt
...
echo "look 1 leaves open: $OPEN"
case "$OPEN" in
  none) echo "# look 1 left no case open" > "$OUT/look2.txt" ;;
  INCOMPLETE*) echo "look 2 not run: wave 1 is incomplete" ;;
  *) NL=$(tr ',' '\n' <<< "$OPEN" | wc -l); NP2=1; [ "$NL" -gt 4 ] && NP2=2
     for k in $(seq 0 $((NP2 - 1))); do
       timeout 28800 node research/solver/audit-s126.mjs bridge7e 16 3000 part $k/$NP2 off,reader "$OPEN" 7011 > "$OUT/look2-part$k.txt" 2>&1 &
     done ;;
esac
...
wait
if ls "$OUT"/look2-part*.txt > /dev/null 2>&1; then cat "$OUT"/look2-part*.txt > "$OUT/look2.txt"; fi
