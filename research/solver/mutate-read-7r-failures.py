...
M = [
    ...
    ("a raise credited on a path that fails", "return { cut, raise: T.survived[i] ? raise : 0 };", "return { cut, raise };"),
    ("a failure in life charged from the year after it", "for (let t = T.failYear[i]; t < T.Y; t++)", "for (let t = T.failYear[i] + 1; t < T.Y; t++)"),
    ("a failure charged as a cut to nothing, not the floor", "cut += lambda * (1 - floor) ** 2", "cut += lambda"),
    ("a raise's credit not capped at 0.2", "Math.sqrt(Math.min(0.2, l - 1))", "Math.sqrt(l - 1)"),
    ("a cut charged linearly, not squared", "cut += lambda * (1 - l) ** 2", "cut += lambda * (1 - l)"),
    ("the cut term's sign reversed", "dCut.push(-100 * (z.cut - a.cut))", "dCut.push(100 * (z.cut - a.cut))"),
    ("the floor read as the menu's highest level", "floor: Math.min(...levels", "floor: Math.max(...levels"),
    ("one minimum pot for every case", "minPot: Number(minPot) };", "minPot: 29000 };"),
    ("a log whose arms disagree accepted", "new Set(ran).size !== 1", "false"),
