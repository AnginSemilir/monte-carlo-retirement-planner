...
mut 'look 2 not used' 's/(open.has(c.id) ? { ...versusOff(look2.find(k => k.id === c.id), pathsL2(c.id), LOOK2), look: 2 } : versusOff(c, 1000, LOOK1))/versusOff(c, 1000, LOOK1)/'
...
mut 'the pooled floor by random effects, not fixed effect' 's/const pool = pooledFE(/const pool = pooledRE(/'
mut 'look 2 at 3,000 for the four long cases' 's/const L2 = 3000, L2_LONG = 8000/const L2 = 3000, L2_LONG = 3000/'
mut 'no long cases' "s/export const LONG = \['bridge 4', 'wealth x0.5', 'S130', 'S128'\];/export const LONG = [];/"
mut 'the gate reads the long cases at 3,000' 's/gate(look2.filter(c => LONG.includes(c.id)), 16, L2_LONG)/gate(look2.filter(c => LONG.includes(c.id)), 16, L2)/'
