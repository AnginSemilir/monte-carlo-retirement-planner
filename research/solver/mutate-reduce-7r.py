...
M = [
    ...
    ("the swap read without Holm over its four comparisons", "const ps = holm(HARMED.flatMap(", "const ps = (a => a)(HARMED.flatMap("),
    ("carrying none read by significance, not the interval", "survivalChange(x.lost, x.saved, x.N, ALPHA).lo > -MARGIN", "!(x.lost > x.saved && x.p < ALPHA)"),
    ("the tier and the rest swapped in the reading", "t && rn ? 'tier' : r && tn ? 'method'", "t && rn ? 'method' : r && tn ? 'tier'"),
    ("a case read though the reader showed no harm there", "reproduced[j] ? reads[j] : null", "reads[j]"),
    ("HELD allowed where both carry the harm", "if (counted.includes('tier') && !counted.some(x => x === 'method' || x === 'both'))", "if (counted.includes('tier') && !counted.some(x => x === 'method'))"),
    ("O23 closed whenever it is not replicated", "iv.lo > -MARGIN ? 'closes: no material harm'", "true ? 'closes: no material harm'"),
    ("the gate ignores a swap arm's difference after the bridge", "else if (w.late !== 0 || w.access !== BRIDGE[id])", "else if (w.access !== BRIDGE[id])"),
    ("the swap line's access year misread", "access: +w[4]", "access: +w[3]"),
