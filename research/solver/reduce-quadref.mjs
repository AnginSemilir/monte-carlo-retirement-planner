...
const EPS = 1e-9;
const verdict = (d, se) => (Math.abs(Math.abs(d) - 2 * se) < EPS ? 'AT THE LINE' : Math.abs(d) > 2 * se ? 'beyond two se' : 'within');
...
{ // planted: one path on which the tier above costs survival with 5 points and not with 15 must give +100/N
  const mk = a => ({ N: 2, paths: { survived: a } }), P = did(mk([1, 1]), mk([0, 1]), mk([1, 1]), mk([1, 1]));
  if (Math.abs(P.d - 50) > EPS) { console.log('PLANTED CHECK FAILED: the difference-in-differences'); process.exit(1); }
  if (verdict(-2, 1) !== 'AT THE LINE' || verdict(-2.01, 1) !== 'beyond two se' || verdict(-1.99, 1) !== 'within') { console.log('PLANTED CHECK FAILED: the tie rule'); process.exit(1); }
}
...
for (const id of ALL) {
  ...
  res[id] = { A, ...q };
  console.log(`  ${id}   ${['d5', 'u5', 'd15', 'u15'].map(k => f(surv(A[k]))).join(' / ')}    ${sg(q.dq.d)} +/- ${f(q.dq.se)} (${verdict(q.dq.d, q.dq.se)}) | ${sg(q.uq.d)} +/- ${f(q.uq.se)} (${verdict(q.uq.d, q.uq.se)})    ${sg(q.c5.d)} +/- ${f(q.c5.se)} | ${sg(q.c15.d)} +/- ${f(q.c15.se)}    ${sg(q.dd.d)} +/- ${f(q.dd.se)}`);
}
...
for (const id of M5) {
  ...
  m5[id] = pair(a, b);
  console.log(`    ${id}   ${f(surv(a))} -> ${f(surv(b))}   ${sg(m5[id].d)} +/- ${f(m5[id].se)} (${verdict(m5[id].d, m5[id].se)})`);
}
...
const beyond = eight.filter(([, x]) => verdict(x.d, x.se) === 'beyond two se'), line = eight.filter(([, x]) => verdict(x.d, x.se) === 'AT THE LINE');
...
console.log(`  2. the three's tier-above cost with 15 points within two se of it with 5 (pooled difference-in-differences): ${sg(DD.d)} +/- ${f(DD.se)} -> ${verdict(DD.d, DD.se) === 'within' ? 'HELD' : verdict(DD.d, DD.se) === 'AT THE LINE' ? 'AT THE LINE' : 'MISSED'}`);
const m5v = M5.filter(id => m5[id]).map(id => [id, verdict(m5[id].d, m5[id].se)]);
...
console.log(`\nFALSIFIER - with the tier above, 15 points raises survival beyond two se, pooled over the three: ${sg(U.d)} +/- ${f(U.se)} -> ${U.d > 0 && verdict(U.d, U.se) === 'beyond two se' ? 'FALSIFIED (the earlier years\' averaging matters)' : U.d > 0 && verdict(U.d, U.se) === 'AT THE LINE' ? 'AT THE LINE' : 'not fired'}`);
const lower = eight.filter(([, x]) => x.d < 0 && verdict(x.d, x.se) === 'beyond two se');
