...
// rule (stats.mjs outcome, Holm across the 16 pool cases - the real rule's Holm runs across 24, so this is a little
// generous to harm), and the pooled floor both ways: random effects (the regimen's) and fixed effect (the maintainer's
...
function run(gains = {}, trials = 4000, seed = 424242) {
  rnd = mulberry32(seed);
  let re = 0, fe = 0, allRE = 0, allFE = 0;
  const gOf = id => (typeof gains === 'number' ? gains : (gains[id] || 0));
  for (let t = 0; t < trials; t++) {
    ...
    const fr = !(pooledRE(rows).lo > -0.1), ff = !(pooledFE(rows).lo > -0.1);
    re += fr; fe += ff; allRE += fr || harm; allFE += ff || harm;
  }
  const pc = x => `${(100 * x / trials).toFixed(1).padStart(5)}%`;
  return `floor RE ${pc(re)}  FE ${pc(fe)}   |   falsifier (harm or floor) RE ${pc(allRE)}  FE ${pc(allFE)}`;
}
