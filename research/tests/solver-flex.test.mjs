...
console.log('=========== C. A FLOOR: THE LANDED POLICY MEETS THE CONFIDENCE ===========');
{
  ...
  /*
   * C4b is the assertion that would have caught the over-trimming landing. A landing that clears the ask
   * by a wide margin is not a safer landing, it is spending given away: the first verification stage
   * shipped in c0862f9 cleared by 2.1 to 7.3 points on three households and halved their years at the
   * full target. Lambda well above the bracket bottom means the solver chose this level of trimming
   * rather than running out of room to trim less.
   */
  const over = floorRate - 100 * ASK;
  ok('C4b the landing does not clear the ask by more than two points, which would be over-trimming',
    over <= 2 || r.lambda <= 6e-3, `over the ask by ${over.toFixed(2)} pts at lambda ${r.lambda.toExponential(2)}`);
