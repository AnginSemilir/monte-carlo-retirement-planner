...
const BLANK_PLAN = Object.freeze({
  ...
  inheritance: {
    ...
    /*
     * Compensation from a government scheme that is disregarded for inheritance tax - the infected blood
     * scheme run by IBCA is the one most people will be entering, and Post Office Horizon, Windrush,
     * Grenfell, the Troubles and vaccine damage payments carry their own exemptions. Held as an amount
...
/*
 * THE TRADE-OFFS A HOUSEHOLD COULD MAKE, PRICED IN THEIR OWN NUMBERS.
 *
 * Ranking six priorities cold asks people to weigh things they have never seen a figure for. The sweep
 * has the figures. So: recommend the survival-first winner, then for every other priority find the
 * candidate that is best on it and say what switching would gain and what it would cost - "£85,000
 * more typical pot, for 1.8 points less survival". A card only exists where the gain clears that
 * priority's own tie threshold, because a difference the ranking itself would call a tie is not a
 * choice, it is noise with a button on it. Nothing outside the survival guard is ever offered.
 *
 * Cards are keyed by candidate, not by priority: one candidate that is best on both pot and bequest is
 * one decision, and listing it twice would make one option look like two.
 *
 * The base is ALWAYS the survival-first ranking, whatever order the household has set, so a card means
 * the same thing every time it appears.
 */
function buildTradeoffs(cands, opts = {}) {
  const empty = { recommended: null, cards: [] };
  if (!Array.isArray(cands) || !cands.length) return empty;
  const tolerances = opts.tolerances;
  const recommended = explainPick(cands, { priorities: DEFAULT_PRIORITIES, tolerances, preAccessCap: opts.preAccessCap, maxSurvivalSacrificePts: opts.maxSurvivalSacrificePts }).winner;
  ...
  const bestOn = (key) => {
    const m = PRIORITY_METRICS[key];
    return [...pool].sort((a, b) => ((m.higherIsBetter ? -1 : 1) * (score(key, a) - score(key, b))) || byFallback(a, b))[0];
  };

  const byId = new Map();
  for (const key of PRIORITY_KEYS) {
    if (key === 'survive') continue;
    // with nobody named as an heir the bequest metric IS the pot metric, and one difference is one card
...
function estateAtDeath(cfg, wrappers, opts = {}) {
  ...
  /*
   * A gift of exempt compensation is not a transfer of value at all: the infected blood scheme gives a
   * recipient two years from payment to pass it on with no inheritance tax, which means no seven-year
...
function estateActionPlan(plan, result) {
  ...
  /*
   * A step the plan already holds is not a step. The order and the ceiling drop out on their own by
   * comparing against the plan; the gifts and the transfers were written with random ids and so never
   * did, which left the list telling a household to make a gift it had already made. Declared up here
   * because the transfers are step 3 and the gifts are step 5.
   */
  const heldGift = (id) => (plan?.inheritance?.gifts || []).some(g => g.id === id);
  const heldRecycle = (b.recycle || []).length > 0
    && (b.recycle || []).every(x => (plan?.oneOffContributions || []).some(c => c.id === x.id));
  ...
  if (b.recycle && b.recycle.length && !heldRecycle) {
 ...
 * anyway, so importing across costs nothing today. It stops being free the moment the two are split -
...
export default function App({ theme = 'system', setTheme = () => {}, resolvedTheme = 'light',
  ...
  /*
   * THE INHERITANCE TAB HAS ITS OWN COPY OF THE PORTFOLIO.
   *
   * It starts as whatever Plan Inputs holds and follows it until somebody types over a balance, at which
   * point the tab keeps its own and says so. Estate planning is a different conversation from "will the
   * money last": it is normal to want to see what happens to the bill if the pension were smaller or the
   * house had already been sold, without any of that leaking into the projection everyone else's numbers
   * come from. One button puts it back.
   *
   * Only balances are overridable. Contributions, growth and retirement ages belong to the projection,
   * and a tab that quietly re-ran the whole plan on different assumptions would be a second plan wearing
   * the first one's name.
   */
  const estateBalancesFromPlan = (pl) => {
    const out = {};
    (pl?.accounts || []).forEach(a => { out[a.id] = E.num(a.balance, 0); });
    return out;
  };
  const [estateBalances, setEstateBalances] = useState(() => estateBalancesFromPlan(plan));
  const [estateBalancesOwn, setEstateBalancesOwn] = useState(false);
  useEffect(() => { if (!estateBalancesOwn) setEstateBalances(estateBalancesFromPlan(plan)); },
    [plan?.accounts, estateBalancesOwn]); // eslint-disable-line react-hooks/exhaustive-deps
  const setEstateBalance = (id, value) => {
    setEstateBalancesOwn(true);
    setEstateBalances(prev => ({ ...prev, [id]: value }));
  };
  ...
  const inheritanceView = useMemo(() => {
    ...
    const at = (age, giftsOverride) => {
       ...
       */
      const row = eRows.find(r => r.ageSelf >= age) || eRows[eRows.length - 1];
      if (!row) return null;
     ...
     */
    const chosenRow = eRows.find(r => r.ageSelf >= chosenAge) || eRows[eRows.length - 1];
    const liquidToday = { cash: 0, other: 0, isa: 0 };
    eCtx.accounts.forEach(a => { if (a.cat in liquidToday) liquidToday[a.cat] += Math.max(0, E.num(a.balance, 0)); });
  ...
  const estateBreakdown = useMemo(() => {
    ...
    const row = eRows.find(r => r.ageSelf >= chosenAge) || eRows[eRows.length - 1];
    if (!row) return null;
    ...
    /*
     * One row per ACCOUNT, not per wrapper, because the balance is editable and an editable field has to
     * belong to something the plan can name. The projection only knows wrapper totals, so where a couple
     * holds two pensions the projected column is split between them in proportion to what they hold
     * today - approximate, and said so on screen rather than presented as a per-account forecast.
     *
    ...
    const accounts = (plan?.accounts || [])
      ...
      .map(a => {
        const cat = catOf(a);
        const bal = Math.max(0, E.num(estateBalances[a.id], 0));
        const share = now[cat] > 0 ? bal / now[cat] : 0;
        return { id: a.id, owner: a.owner, category: a.category, cat, balance: bal, at: at[cat] * share };
      });
    const splitAcross = E.CATEGORIES.some(k => accounts.filter(a => a.cat === k).length > 1);
    return {
      chosenAge, year: row.year, wrappers, assets, accounts, splitAcross, pensionCounted: counted,
...
<footer>Every route was run through the same projection and priced at the same death age; ${esc(estatePlan.runs)} of them in all. Figures are in today\u2019s money and follow the tax rules set on the Config tab. This models tax only: it cannot draft a will, witness a signature, or tell you whether a gift is wise for reasons that have nothing to do with tax.</footer>
  ...
  // the one place a policy candidate becomes the plan's settings: the sweep's winner and a trade-off
  // card both go through here, so neither can change a field the other does not
  const applyCandidate = (c) => setPlan(prev => ({
    ...prev,
    spending: { ...(prev.spending || {}), decumulationPolicy: c.decumulationPolicy, drawdownStrategy: c.drawdownStrategy },
    config: { ...(prev.config || {}), harvestPersonalAllowance: c.harvestPersonalAllowance }
  }));
  const chooseTradeoff = (card) => {
    applyCandidate(card.candidate);
    setPolicyResults(r => (r ? { ...r, chosenId: card.candidate.id } : r));
    flash(`Applied "${card.candidate.label}"`, 3000);
  };
        ...
        {activeTab === 'config' && (
          <div className="space-y-6">
            ...
            <div className={isPhone ? 'space-y-3 pb-2' : 'bg-surface border border-slate-200/90 p-5 rounded-xl space-y-4'}>
              ...
              {/* What used to be six draggable priorities is now a recommendation plus priced trade-offs
                  (rendered with the results below). The ranking still exists, folded, for anyone who
                  genuinely has an order: measured across 420 households, promoting the pot to first
                  changes the recommendation for 75% of them, so it is a real control, just not the
                  one most people should be asked to operate cold. */}
              {!policyResults && !isPolicySearching && (
                <p className="text-[11px] text-slate-600 leading-relaxed">Run the policy search to see the recommended settings, and any alternatives that would trade some safety for a materially better result on something else.</p>
              )}
              <details className="pt-1 text-xs">
                <summary className="cursor-pointer text-slate-600 font-semibold hover:text-slate-900">Advanced: rank the priorities yourself</summary>
              <div className="mt-2">
                ...
                <span className="text-[10px] text-slate-400 mt-1.5 block">
                  Drag a row to move it, or use the arrows. Worked down in order. A lower priority only decides between options that are already within {E.RATE_EPSILON_PTS} percentage point (survival, bridge risk) or {Math.round(E.MONEY_EPSILON_REL * 100)}% (money) of the best on every priority above it (or your own thresholds, set below) — so nothing you rank higher is ever traded away for something you rank lower.
                </span>
              ...
              {policyResults && (
                <div className="pt-3 border-t border-slate-100 space-y-2">
                  ...
                  {/*
                    ...
                    * recommendation on something by more than that something's own tie threshold,
                    * with what it costs in survival and anything else. No card means no candidate
                    * offers a real difference, which is worth saying in so many words: the absence
                    * is the finding.
                    */}
                  {policyResults.tradeoffs && (() => {
                    const { recommended, cards } = policyResults.tradeoffs;
                    if (!recommended) return null;
                    const rec = policyResults.rows.find(r => r.id === recommended.id) || recommended;
                    const chosen = policyResults.chosenId;
                    const money = (d, from) => `${formatGBP(Math.abs(d))}${Math.abs(from) > 0 ? ` (${Math.round(100 * Math.abs(d) / Math.abs(from))}%)` : ''}`;
                    const GAIN = {
                      pot: (g) => `${money(g.delta, g.from)} more in the typical pot at your final age`,
                      bequest: (g) => `${money(g.delta, g.from)} more for your heirs after tax`,
                      downside: (g) => `${money(g.delta, g.from)} more left in the worst one lifetime in ten`,
                      tax: (g) => `${money(g.delta, g.from)} less lifetime tax`,
                      bridge: (g) => `${g.delta.toFixed(1)} points less risk of running dry before pension access`
                    };
                    const COST = {
                      pot: (c) => `${money(c.delta, c.from)} less in the typical pot`,
                      bequest: (c) => `${money(c.delta, c.from)} less for your heirs`,
                      downside: (c) => `${money(c.delta, c.from)} less in the worst one lifetime in ten`,
                      tax: (c) => `${money(c.delta, c.from)} more lifetime tax`,
                    ...
                    <table className="w-full text-left text-[11px] border-collapse">
                      ...
                      <tbody className="divide-y divide-slate-100 font-mono">
                        {policyResults.rows.map(r => {
                          ...
                          const chosen = r.id === policyResults.chosenId && !won;
                          return (
                            <tr key={r.id} className={won ? 'bg-emerald-50/70' : chosen ? 'bg-blue-50/70' : 'hover:bg-slate-50/80'}>
                              <td className={`py-1.5 pr-3 font-sans ${won ? 'font-bold text-emerald-900' : chosen ? 'font-bold text-blue-900' : 'text-slate-700'}`}>{won && <Trophy className="w-3 h-3 text-emerald-600 inline mr-1 -mt-0.5" />}{chosen && <Check className="w-3 h-3 text-blue-600 inline mr-1 -mt-0.5" />}{r.label}</td>
                  ...
                  <table className="w-full text-left text-[11px] border-collapse">
                    ...
                    <tbody className="divide-y divide-slate-100 font-mono">
                      {/* editable, because estate planning asks "what if the pension were smaller" and the
                          answer must not disturb the projection everybody else's numbers come from */}
                      {estateBreakdown.accounts.map(a => {
                        const cat = a.cat;
                        return (
                          <tr key={a.id}>
                            ...
                            <td className="py-1.5 pr-3 text-slate-800 font-bold">{formatGBP(a.at)}</td>
                            <td className="py-1.5 font-sans text-[10px] text-slate-500">
                              {cat === 'pen'
                                ? (estateBreakdown.pensionCounted ? `Yes, from ${E.num(plan?.config?.pensionsInEstateFrom, 2027)}` : `No — death before ${E.num(plan?.config?.pensionsInEstateFrom, 2027)}`)
                                : 'Yes'}
                            </td>
                          </tr>
                        );
                      ...
                      <tr className="border-t-2 border-slate-200">
                        ...
                        <td className="py-1.5 pr-3 text-slate-400">{formatGBP(estateBreakdown.accounts.reduce((t, a) => t + a.balance, 0) + estateBreakdown.homeNow + estateBreakdown.assetsValue)}</td>
